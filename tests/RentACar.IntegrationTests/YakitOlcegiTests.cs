...
{
    private static readonly DateTimeOffset Start = TestZaman.DaysLater(1);
    private static readonly DateTimeOffset Bit = TestZaman.DaysLater(5);
    ...
    public void Yuzde_on_ikiye(int percent, int expected) => Assert.Equal(expected, FuelScale.PercentToTwelfths(percent));
    ...
    [InlineData(12, 100)]
    public void On_ikiden_yuzdeye(int inTwelfths, int expected) => Assert.Equal(expected, FuelScale.TwelfthsToPercent(inTwelfths));

    // Kira servisi müşteri/araç varlığını girişte doğrular (#328) → her kira kiracıda GERÇEK araç + cari ile.
    private static async Task<BookingInput> InputAsync(IServiceProvider sp, decimal fuelUnit)
        => Input(await TestVehicle.NewAsync(sp), await TestCustomer.NewAsync(sp), fuelUnit);

    private static BookingInput Input(Guid vehicle, Guid account, decimal fuelUnit) => new()
    {
        MusteriId = account, VehicleId = vehicle, BasTar = Start, BitTar = Bit,
        GunlukUcret = 100m, KmLimit = 0, FazlaKmUcret = 0m, YakitBirimUcret = fuelUnit
    };
    ...
    public async Task Eksik_yakit_bedeli_on_ikide_bir_birimle()
    {
        ...
        var id = await svc.CreateDirectAsync(await InputAsync(scope.ServiceProvider, fuelUnit: 150m));
    ...
    public async Task Servis_12_ustunu_ve_negatifi_reddeder()
    {
        ...
        var id = await svc.CreateDirectAsync(await InputAsync(scope.ServiceProvider, fuelUnit: 10m));
    ...
    private async Task<(int? Cikis, int? Donus)> RawFuelAsync(Guid tenant, Guid rentalId)
    {
    ...
    public async Task Harici_api_yuzde_alir_on_ikide_bir_saklar_yuzde_dondurur()
    {
        ...
        var rental = await IdAsync(await c.PostAsJsonAsync("/api/v1/rentals",
            new { musteriId = mId, vehicleId = vId, basTar = start, bitTar = bit, gunlukUcret = 100m, yakitBirimUcret = 100m }));
        // Birim ücret sınırda yüzde puanı başından on ikide bir başına: 100 × 100/12 = 833,3333 (4 hane)
        Assert.Equal(833.3333m, await RawUnitAsync(tenant, rental));
        var created = await c.GetFromJsonAsync<JsonElement>($"/api/v1/rentals/{rental}");
        Assert.Equal(100m, created.GetProperty("yakitBirimUcret").GetDecimal()); // 833,3333 × 12/100 = 99,999996 → 100,00

        // 101 / -1 → 400; hiçbir şey yazılmaz
        Assert.Equal(HttpStatusCode.BadRequest,
            (await c.PostAsJsonAsync($"/api/v1/rentals/{rental}/deliver", new { cikisKm = 1000, cikisYakit = 101 })).StatusCode);
        Assert.Equal(HttpStatusCode.BadRequest,
            (await c.PostAsJsonAsync($"/api/v1/rentals/{rental}/deliver", new { cikisKm = 1000, cikisYakit = -1 })).StatusCode);
        Assert.Equal((null, null), await RawFuelAsync(tenant, rental));

        // %50 → 6 saklanır → %50 okunur
        var delivery = await c.PostAsJsonAsync($"/api/v1/rentals/{rental}/deliver", new { cikisKm = 1000, cikisYakit = 50 });
        Assert.Equal(HttpStatusCode.OK, delivery.StatusCode);
        Assert.Equal((6, (int?)null), await RawFuelAsync(tenant, rental));
        var read = await c.GetFromJsonAsync<JsonElement>($"/api/v1/rentals/{rental}");
        Assert.Equal(50, read.GetProperty("cikisYakit").GetInt32());

        Assert.Equal(HttpStatusCode.BadRequest, (await c.PostAsJsonAsync($"/api/v1/rentals/{rental}/return",
        ...
        var returnInfo = await c.PostAsJsonAsync($"/api/v1/rentals/{rental}/return", new { donusKm = 1000, donusYakit = 25, gercekDonus = bit });
        Assert.Equal(HttpStatusCode.OK, returnInfo.StatusCode);
        Assert.Equal(((int?)6, (int?)3), await RawFuelAsync(tenant, rental));
        var j = await returnInfo.Content.ReadFromJsonAsync<JsonElement>();
        ...
        var rental2 = await IdAsync(await c.PostAsJsonAsync("/api/v1/rentals",
            new { musteriId = mId, vehicleId = vId2, basTar = start, bitTar = bit, gunlukUcret = 100m, yakitBirimUcret = 10m }));
        Assert.Equal(83.3333m, await RawUnitAsync(tenant, rental2));
        Assert.Equal(HttpStatusCode.OK,
            (await c.PostAsJsonAsync($"/api/v1/rentals/{rental2}/deliver", new { cikisKm = 1000, cikisYakit = 100 })).StatusCode);
        var j2 = await (await c.PostAsJsonAsync($"/api/v1/rentals/{rental2}/return",
        ...
        var res = await c.PostAsJsonAsync("/api/v1/reservations", new
        {
            musteriId = mId, vehicleId = vId, basTar = bit.AddDays(5), bitTar = bit.AddDays(7), gunlukUcret = 50m, yakitBirimUcret = 10m
        });
        Assert.Equal(HttpStatusCode.Created, res.StatusCode);
        Assert.Equal(10m, (await res.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("yakitBirimUcret").GetDecimal());
    ...
    private async Task<decimal> RawUnitAsync(Guid tenant, Guid rentalId)
    {
        await using var con = new NpgsqlConnection(fx.OwnerConnectionString);
        await con.OpenAsync();
        await using var tx = await con.BeginTransactionAsync();
    ...
    private async Task WriteRawAsync(Guid tenant, string sql, Guid id)
    {
        await using var con = new NpgsqlConnection(fx.OwnerConnectionString);
        await con.OpenAsync();
        await using var tx = await con.BeginTransactionAsync();
    ...
    public async Task Migration_yuzdeleri_on_ikide_bire_cevirir()
    {
        ...
        {
            var code = $"ym{Guid.NewGuid():N}";
            db.Tenants.Add(new Tenant { Id = tenant, Code = code, Name = code, IsActive = true });
            await db.SaveChangesAsync();
        }

        Guid percentRental, twelfthsRental, closedRental, baf;
        using (var host = new TestHost(fx.AppConnectionString))
        using (var scope = host.ScopeFor(tenant))
        {
            var rentals = scope.ServiceProvider.GetRequiredService<RentalService>();
            percentRental = await rentals.CreateDirectAsync(await InputAsync(scope.ServiceProvider, 10m));
            twelfthsRental = await rentals.CreateDirectAsync(await InputAsync(scope.ServiceProvider, 10m));
            closedRental = await rentals.CreateDirectAsync(await InputAsync(scope.ServiceProvider, 10m));
        ...
        await WriteRawAsync(tenant, "UPDATE \"Rentals\" SET \"CikisYakit\" = 80, \"DonusYakit\" = 45 WHERE \"Id\" = @id", percentRental);
        await WriteRawAsync(tenant, "UPDATE \"Rentals\" SET \"CikisYakit\" = 8, \"DonusYakit\" = -3 WHERE \"Id\" = @id", twelfthsRental);
        await WriteRawAsync(tenant, "UPDATE \"Rentals\" SET \"CikisYakit\" = 80, \"DonusYakit\" = 40, \"Durum\" = 1 WHERE \"Id\" = @id", closedRental);
        await WriteRawAsync(tenant, "UPDATE \"Baflar\" SET \"CikisYakit\" = 80, \"DonusYakit\" = 8 WHERE \"Id\" = @id", baf);
        ...
        const string loop = "FOR t IN SELECT \"Id\" FROM \"Tenants\" LOOP";
        Assert.Contains(loop, sql);
        sql = sql.Replace(loop, $"FOR t IN SELECT '{tenant}'::uuid LOOP");
        await using (var con = new NpgsqlConnection(fx.OwnerConnectionString))
        {
            await con.OpenAsync();
        ...
        Assert.Equal(((int?)10, (int?)5), await RawFuelAsync(tenant, percentRental)); // 80→9,6→10; 45→5,4→5
        Assert.Equal(((int?)8, (int?)0), await RawFuelAsync(tenant, twelfthsRental));  // 0–12 dokunulmaz; negatif → 0
        Assert.Equal(((int?)10, (int?)5), await RawFuelAsync(tenant, closedRental)); // 40 → 4,8 → 5
        // Birim ücret: YALNIZ açık (Kirada) + yüzdeyle teslim edilmiş kirada çevrilir: 10 × 100/12 = 83,3333.
        Assert.Equal(83.3333m, await RawUnitAsync(tenant, percentRental));
        Assert.Equal(10m, await RawUnitAsync(tenant, twelfthsRental));   // zaten on ikide bir ölçeği
        Assert.Equal(10m, await RawUnitAsync(tenant, closedRental));  // kapanmış: faturalanmış geçmiş, dokunulmaz
        using (var host = new TestHost(fx.AppConnectionString))
        using (var scope = host.ScopeFor(tenant))
        {
