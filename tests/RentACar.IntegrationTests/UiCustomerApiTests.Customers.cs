...
{
    private static Dictionary<string, object?> CustomerBody(string tc, string surname = "Kaya") => new()
    {
        ["tip"] = "Bireysel", ["ad"] = "Ece", ["soyad"] = surname, ["tcKimlik"] = tc, ["cepTel"] = "05321112233",
    ...
    public async Task Customer_create_update_version_and_tc_never_returned()
    {
        ...
        var (created, raw) = await Json(await Send(admin, HttpMethod.Post, Customers, CustomerBody(tc)), HttpStatusCode.Created);
        ...
        var (full, fullRaw) = await Json(await Send(admin, HttpMethod.Get, $"{Customers}?q={tc}"));
        Assert.Equal(id, Assert.Single(Records(full)).GetProperty("id").GetGuid());
        Assert.DoesNotContain(tc, fullRaw);
        var (partial, _) = await Json(await Send(admin, HttpMethod.Get, $"{Customers}?q={tc[..6]}"));
        Assert.Empty(Records(partial));

        // Aynı TC ikinci kez → 400 errors[tcKimlik], mesajda TC yok; geçersiz TC → 400.
        var dup = await Problem(await Send(admin, HttpMethod.Post, Customers, CustomerBody(tc, "Başka")), HttpStatusCode.BadRequest, "dogrulama", "tcKimlik");
        Assert.DoesNotContain(tc, dup);
        var bad = tc[..10] + ((tc[10] - '0' + 1) % 10);
        ...
        Assert.DoesNotContain(tc, updatedRaw);
        ...
        var (stillFound, _) = await Json(await Send(admin, HttpMethod.Get, $"{Customers}?q={tc}"));
        ...
        var (gone, _) = await Json(await Send(admin, HttpMethod.Get, $"{Customers}?q={tc}"));
        Assert.Empty(Records(gone));

        // Sınırlar: uzun metin ve taşan tutar 400 + alan.
        var tooLong = CustomerBody(RandomTc());
        ...
        var tooBig = CustomerBody(RandomTc());
        ...
        var badType = CustomerBody(RandomTc());
        badType["tip"] = "Uzaylı";
    ...
    public async Task Customer_anonymized_groups_are_hidden_and_preserved()
    {
        ...
        var admin = await LoginAsync(e, Who.Admin);
        var tc = RandomTc();
        var body = CustomerBody(tc, "Gizlioglu");
        body["anonimAd"] = true;
        ...
        foreach (var hidden in new[] { tc, "Gizlioglu", "05321112233" }) Assert.DoesNotContain(hidden, raw);
    ...
    public async Task Customer_permissions_delete_guard_and_tenant_isolation()
    {
        ...
        var (c, _) = await Json(await Send(admin, HttpMethod.Post, Customers, CustomerBody(RandomTc())), HttpStatusCode.Created);
        ...
        await Problem(await Send(acc, HttpMethod.Post, Customers, CustomerBody(RandomTc())), HttpStatusCode.Forbidden, "yetki_yok");
        ...
        var (free, _) = await Json(await Send(admin, HttpMethod.Post, Customers, CustomerBody(RandomTc())), HttpStatusCode.Created);
        ...
        var (foreign, _) = await Json(await Send(otherAdmin, HttpMethod.Post, Customers, CustomerBody(RandomTc(), "Yabanci")), HttpStatusCode.Created);
        ...
        var put = CustomerBody(RandomTc(), "Ele");
