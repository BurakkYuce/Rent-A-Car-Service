...
public sealed partial class UiAracTests
{
    ...
    public async Task Legacy_out_of_range_values_do_not_block_unrelated_edit_but_changed_invalid_value_is_400()
    {
        ...
        {
            Plaka = Plate("34E"), Marka = "Murat", Tip = "124", Grup = "C", Sube = "SubeA", Km = 1000,
            TescilTarihi = new DateTimeOffset(1940, 6, 15, 0, 0, 0, TimeSpan.Zero), AlisOtv = -5m,
        };
        await WriteDataAsync(o.TenantId, db => db.Vehicles.Add(legacy)); // servis doğrulamasını atlar (eski veri)

        async Task<Dictionary<string, JsonElement>> CardAsync()
            => JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(
                (await Json(await Gonder(admin, HttpMethod.Get, $"{SampleVehicle}/{legacy.Id}"))).GetRawText())!;
        ...
        var ok = await Json(await Gonder(admin, HttpMethod.Put, $"{SampleVehicle}/{legacy.Id}", card));
        ...
        await Json(await Gonder(admin, HttpMethod.Put, $"{SampleVehicle}/{legacy.Id}", card));

        // 3) Alan başka bir geçersiz değere çevrilirse → 400 alanlı.
        card = await CardAsync();
        card["tescilTarihi"] = J("1939-01-01T00:00:00Z");
        await ExpectProblem(await Gonder(admin, HttpMethod.Put, $"{SampleVehicle}/{legacy.Id}", card),
        ...
        card["alisOtv"] = J(0m);
        var fixedCard = await Json(await Gonder(admin, HttpMethod.Put, $"{SampleVehicle}/{legacy.Id}", card));
