...
public sealed class CrmAnalizTests(PostgresFixture fx)
{
    ...
    public async Task Segment_yeni_alanlar_ve_ortalama_kira_bedeli()
    {
        ...
        await rentals.CreateDirectAsync(new BookingInput
        { MusteriId = a, VehicleId = v1, BasTar = bas, BitTar = bas.AddDays(3), GunlukUcret = 100m, KmLimit = 0, FazlaKmUcret = 0m, Kaynak = "Web" });
        // A'nın 2. kirası: 2 gün × 100 = 200
        await rentals.CreateDirectAsync(new BookingInput
        { MusteriId = a, VehicleId = v2, BasTar = bas.AddDays(30), BitTar = bas.AddDays(32), GunlukUcret = 100m, KmLimit = 0, FazlaKmUcret = 0m, Kaynak = "Acente" });
        // B: 5 gün × 100 = 500
        await rentals.CreateDirectAsync(new BookingInput
        ...
        Assert.Equal(bas, ra.IlkKiraZamani);          // en erken BasTar
        Assert.Equal(bas.AddDays(30), ra.SonIslem);   // en geç BasTar
    ...
    public async Task Segment_filtreleri_tarih_adet_kaynak_ofis()
    {
        ...
        await rentals.CreateDirectAsync(new BookingInput
        { MusteriId = a, VehicleId = v2, BasTar = mayis, BitTar = mayis.AddDays(2), GunlukUcret = 100m, KmLimit = 0, FazlaKmUcret = 0m, Kaynak = "Acente", CikisOfisi = "Havalimanı" });
        await rentals.CreateDirectAsync(new BookingInput
        ...
        Assert.Contains("Havalimanı", sec.Ofisler);
    }
    ...
    public async Task Segment_tenant_izolasyonu()
    {
        ...
        using (var s1 = host.ScopeFor(Guid.NewGuid()))
        {
            ...
            { MusteriId = c, VehicleId = v, BasTar = bas, BitTar = bas.AddDays(2), GunlukUcret = 100m, KmLimit = 0, FazlaKmUcret = 0m, Kaynak = "Web" });
        }
