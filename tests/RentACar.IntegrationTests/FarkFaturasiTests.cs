...
public sealed class FarkFaturasiTests(PostgresFixture fx)
{
    ...
    private static async Task<Guid> ExchangeRateRentalAsync(IServiceProvider sp, string plate)
    {
        ...
        { MusteriId = m, VehicleId = v, BasTar = Start, BitTar = Start.AddDays(3), GunlukUcret = 100m, KmLimit = 300, FazlaKmUcret = 2m });
    }
    ...
    public async Task Donusten_once_fatura_sonra_fark_defteri_kapatir()
    {
        ...
        var id = await ExchangeRateRentalAsync(sp, "34 FK 01");
        var customerId = (await rentals.GetAsync(id))!.MusteriId;

        // 1) Dönüşten ÖNCE faturala → baz 300; cari borç 300.
        await invoices.CreateFromRentalAsync(id);
        Assert.Equal(300m, await cash.GetAccountBalanceAsync(customerId));
        ...
        await rentals.ReturnAsync(id, returnKm: 1600, returnFuel: 8, Start.AddDays(3));
        ...
        await invoices.CreateFromRentalAsync(id);
        Assert.Equal(900m, await cash.GetAccountBalanceAsync(customerId));
    ...
    public async Task Fark_faturasi_rentalId_null_kaynakKira_dolu()
    {
        ...
        var rentals = sp.GetRequiredService<RentalService>();
        var invoices = sp.GetRequiredService<InvoiceService>();
        var id = await ExchangeRateRentalAsync(sp, "34 FK 02");

        await invoices.CreateFromRentalAsync(id);
        await rentals.DeliverAsync(id, pickupKm: 0, pickupFuel: 8);
        await rentals.ReturnAsync(id, returnKm: 500, returnFuel: 8, Start.AddDays(3)); // 200 aşım × 2 = 400
        var differenceId = await invoices.CreateFromRentalAsync(id);

        var difference = await sp.GetRequiredService<IInvoiceRepository>().FindAsync(differenceId);
        Assert.Null(difference!.RentalId);           // kira-fatura unique index'ine çarpmaz
        Assert.Equal(id, difference.KaynakKiraId);   // kira bağı
        Assert.Equal(400m, difference.GenelToplam);  // 200 km × 2
    }
    ...
    public async Task Escantli_cift_fark_idempotent_tek_yazar()
    {
        ...
        {
            var sp = scope.ServiceProvider;
            id = await ExchangeRateRentalAsync(sp, "34 FK 04");
            customerId = (await sp.GetRequiredService<RentalService>().GetAsync(id))!.MusteriId;
            ...
            await sp.GetRequiredService<RentalService>().ReturnAsync(id, returnKm: 1600, returnFuel: 8, Start.AddDays(3));
        }
        ...
        await Task.WhenAll(Enumerable.Range(0, 10).Select(_ => Task.Run(async () =>
        {
            ...
            try { await s.ServiceProvider.GetRequiredService<InvoiceService>().CreateFromRentalAsync(id); Interlocked.Increment(ref success); }
            catch (ValidationException) { /* idempotent red beklenir */ }
        })));

        using var check = host.ScopeFor(tenant);
        Assert.Equal(1, success);                                                            // yalnız 1 fark başardı
        Assert.Equal(900m, await check.ServiceProvider.GetRequiredService<CashService>().GetAccountBalanceAsync(customerId)); // 12× değil
    }
    ...
    public async Task Iade_sonrasi_donus_farki_defteri_hizalar()
    {
        ...
        var invoices = sp.GetRequiredService<InvoiceService>();
        var cash = sp.GetRequiredService<CashService>();
        var id = await ExchangeRateRentalAsync(sp, "34 FK 05");
        var customerId = (await rentals.GetAsync(id))!.MusteriId;

        var bazId = await invoices.CreateFromRentalAsync(id);        // base 300
        await invoices.CreateRefundAsync(bazId);                        // iade → cari 0
        Assert.Equal(0m, await cash.GetAccountBalanceAsync(customerId));
        await rentals.DeliverAsync(id, pickupKm: 1000, pickupFuel: 8);
        await rentals.ReturnAsync(id, returnKm: 1600, returnFuel: 8, Start.AddDays(3)); // sözleşme 900
        await invoices.CreateFromRentalAsync(id);                     // fark = 900 − 0 (iade netlendi)
        Assert.Equal(900m, await cash.GetAccountBalanceAsync(customerId));   // defter = sözleşme
    }
    ...
    public async Task Iade_sonrasi_yeniden_faturalanabilir()
    {
        ...
        var invoices = sp.GetRequiredService<InvoiceService>();
        var cash = sp.GetRequiredService<CashService>();
        var id = await ExchangeRateRentalAsync(sp, "34 FK 06");
        var customerId = (await sp.GetRequiredService<RentalService>().GetAsync(id))!.MusteriId;
    ...
    public async Task Fark_iade_edilince_yeniden_kesilebilir()
    {
        ...
        var invoices = sp.GetRequiredService<InvoiceService>();
        var cash = sp.GetRequiredService<CashService>();
        var id = await ExchangeRateRentalAsync(sp, "34 FK 07");
        var customerId = (await rentals.GetAsync(id))!.MusteriId;

        await invoices.CreateFromRentalAsync(id);                    // base 300
        await rentals.DeliverAsync(id, pickupKm: 1000, pickupFuel: 8);
        await rentals.ReturnAsync(id, returnKm: 1600, returnFuel: 8, Start.AddDays(3)); // sözleşme 900
        var difference1 = await invoices.CreateFromRentalAsync(id);        // fark 600 (sıra 1) → cari 900
        Assert.Equal(900m, await cash.GetAccountBalanceAsync(customerId));

        await invoices.CreateRefundAsync(difference1);                        // fark'ı iade et → cari 300
        Assert.Equal(300m, await cash.GetAccountBalanceAsync(customerId));

        await invoices.CreateFromRentalAsync(id);                     // yeniden fark 600 (sıra 2) → cari 900
        Assert.Equal(900m, await cash.GetAccountBalanceAsync(customerId));   // defter = sözleşme (kilitlenmedi)
    }
    ...
    public async Task Donussuz_tek_fatura_hala_calisir()
    {
        ...
        var invoices = sp.GetRequiredService<InvoiceService>();
        var cash = sp.GetRequiredService<CashService>();
        var id = await ExchangeRateRentalAsync(sp, "34 FK 03");
        var customerId = (await sp.GetRequiredService<RentalService>().GetAsync(id))!.MusteriId;
