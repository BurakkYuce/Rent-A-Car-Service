...
public sealed class SikayetTeslimBagliTests(PostgresFixture fx)
{
    ...
    private static async Task<Guid> CariAsync(IServiceProvider sp, string unvan, string? tel = null)
        ...
        { Tip = CariType.Kurumsal, Unvan = unvan, CepTel = tel });
    ...
    public async Task Yeni_alanlar_round_trip_ve_sozlesme_bilgisi_COZULUR()
    {
        ...
        var personel = sp.GetRequiredService<PersonelService>();
        ...
        var svc = sp.GetRequiredService<SikayetService>();
        var id = await svc.CreateAsync(new SikayetInput
        {
            ...
            Puan = 2, SikayetKanali = " Telefon ", SikayetYeri = SikayetYeri.Kira,
        ...
        Assert.Equal(SikayetYeri.Kira, r.Sikayet.SikayetYeri);
    ...
    public async Task Plaka_SNAPSHOT_DEGIL_arac_plakasi_degisince_liste_de_degisir()
    {
        ...
        await sp.GetRequiredService<SikayetService>().CreateAsync(new SikayetInput
        { CariId = cari, Konu = "Test", RentalId = kira });

        Assert.Equal("34ES01", Assert.Single(await sp.GetRequiredService<SikayetService>().SearchAsync()).Plaka);
        ...
        Assert.Equal("06YN02", Assert.Single(await sp.GetRequiredService<SikayetService>().SearchAsync()).Plaka);
    }
    ...
    public async Task Sozlesmesiz_genel_sikayet_ESKI_davranis_korunur()
    {
        ...
        Assert.Equal(SikayetDurum.Acik, r.Sikayet.Durum);
    }
    ...
    public async Task Filtreler_ELLE_BEKLENEN_alt_kumeyi_dondurur()
    {
        ...
        { CariId = a, Konu = "Gecikme", RentalId = kiraA, CikisOfisi = "Merkez",
          SikayetKanali = "Telefon", SikayetYeri = SikayetYeri.Kira });
        await svc.CreateAsync(new SikayetInput
        { CariId = b, Konu = "Yanlış araç", CikisOfisi = "Şube2",
          SikayetKanali = "Web", SikayetYeri = SikayetYeri.Rezervasyon, Durum = SikayetDurum.Kapali });
        await svc.CreateAsync(new SikayetInput
        ...
        Assert.Single(await svc.SearchAsync(new SikayetFilter { Yer = SikayetYeri.Rezervasyon }));
        Assert.Single(await svc.SearchAsync(new SikayetFilter { Durum = SikayetDurum.Kapali }));
    ...
    public async Task Sikayet_tenant_izolasyonlu()
    {
        ...
        var svc = s2.ServiceProvider.GetRequiredService<SikayetService>();
