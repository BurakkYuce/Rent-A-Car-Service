...
public sealed class KabukTests
{
    ...
    /// </summary>
    private static string Rule(string css, string picker)
    {
        var i = css.IndexOf("\n" + picker + " {", StringComparison.Ordinal);
        Assert.True(i >= 0, $"`{picker}` kuralı app.css'te (satır başında) bulunamadı.");
        var last = css.IndexOf('}', i);
        return css[i..last];
    }
    ...
    {
        var css = Css();
        Assert.Contains("overflow: hidden", Rule(css, ".app"), StringComparison.Ordinal);
        Assert.Contains("overflow-y: auto", Rule(css, ".main"), StringComparison.Ordinal);
    }
    ...
    public void Yazdirmada_app_shell_cozulur()
    {
        ...
        var block = css[i..css.IndexOf("\n}", i, StringComparison.Ordinal)];

        Assert.Contains(".app", block, StringComparison.Ordinal);
        Assert.Contains(".main", block, StringComparison.Ordinal);
        Assert.Contains("height: auto !important", block, StringComparison.Ordinal);
        Assert.Contains("overflow: visible !important", block, StringComparison.Ordinal);
    }
}
