...
public sealed class IkinciSurucuTests(PostgresFixture fx)
{
    ...
    private static async Task<(Guid cari, Guid veh)> SeedAsync(IServiceProvider sp, string plate, string name)
    {
        var account = await sp.GetRequiredService<CustomerService>()
            .CreateAsync(new CustomerInput { Tip = CustomerType.Bireysel, Ad = name, Soyad = "S" });
    ...
    public async Task Ikinci_surucu_sozlesmede_gorunur()
    {
        ...
        var (account, veh) = await SeedAsync(sp, "34 IS 01", "Birinci");
        var second = await sp.GetRequiredService<CustomerService>().CreateAsync(new CustomerInput
        ...
        { MusteriId = account, IkinciSurucuId = second, VehicleId = veh, BasTar = Start, BitTar = Start.AddDays(2), GunlukUcret = 100m });
    ...
    public async Task Ikinci_surucu_null_tek_surucu()
    {
        ...
        using var scope = host.ScopeFor(Guid.NewGuid());
        var sp = scope.ServiceProvider;
        var (account, veh) = await SeedAsync(sp, "34 IS 02", "Tek");
        var rental = await sp.GetRequiredService<RentalService>().CreateDirectAsync(new BookingInput
    ...
    public async Task Ikinci_surucu_musteriyle_ayni_reddedilir()
    {
        ...
        using var scope = host.ScopeFor(Guid.NewGuid());
        var sp = scope.ServiceProvider;
        var (account, veh) = await SeedAsync(sp, "34 IS 03", "Ayni");
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
            sp.GetRequiredService<RentalService>().CreateDirectAsync(new BookingInput
            { MusteriId = account, IkinciSurucuId = account, VehicleId = veh, BasTar = Start, BitTar = Start.AddDays(2), GunlukUcret = 100m }));
    ...
    public async Task Olmayan_ikinci_surucu_reddedilir()
    {
        ...
        using var scope = host.ScopeFor(Guid.NewGuid());
        var sp = scope.ServiceProvider;
        var (account, veh) = await SeedAsync(sp, "34 IS 04", "Var");
        // Başka tenant'ın / var olmayan id → RLS zaten keser; erken temiz red.
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
            sp.GetRequiredService<RentalService>().CreateDirectAsync(new BookingInput
            { MusteriId = account, IkinciSurucuId = Guid.NewGuid(), VehicleId = veh, BasTar = Start, BitTar = Start.AddDays(2), GunlukUcret = 100m }));
