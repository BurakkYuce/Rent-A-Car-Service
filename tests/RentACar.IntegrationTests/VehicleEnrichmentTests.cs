...
public sealed class VehicleEnrichmentTests(PostgresFixture fx)
{
    ...
    public async Task Parite_fields_roundtrip()
    {
        ...
        var registration = new DateTimeOffset(2021, 3, 15, 0, 0, 0, TimeSpan.Zero);
        var purchase = new DateTimeOffset(2021, 2, 1, 0, 0, 0, TimeSpan.Zero);
        var entry = new DateTimeOffset(2021, 2, 5, 0, 0, 0, TimeSpan.Zero);
        var id = await svc.CreateAsync(new VehicleInput
        {
            Plaka = "34 PAR 01", AracSahibi = "Yüce Filo A.Ş.",
            MotorGucu = 150, SilindirHacmi = 1598, RuhsatNo = "RZ-7788", TescilTarihi = registration,
            AlimBedeli = 850000.00m, AlimTarihi = purchase,
        ...
        Assert.Equal(registration, v.TescilTarihi);
        Assert.Equal(850000.00m, v.AlimBedeli);
        Assert.Equal(purchase, v.AlimTarihi);
        ...
        Assert.Equal(entry, v.FiloGirisTarih);
    ...
    public async Task Model_yili_ust_sinir_gelecek_yil()
    {
        ...
        var nextYear = DateTimeOffset.UtcNow.Year + 1; // oracle: yeni model araç en fazla gelecek yıl olabilir

        var id = await svc.CreateAsync(new VehicleInput { Plaka = "34 YIL 02", ModelYili = nextYear });
        Assert.Equal(nextYear, (await svc.GetAsync(id))!.ModelYili); // gelecek yıl → geçerli

        await Assert.ThrowsAsync<ValidationException>( // gelecek yıl + 1 → reddedilir
            () => svc.CreateAsync(new VehicleInput { Plaka = "34 YIL 03", ModelYili = nextYear + 1 }));
