...
public sealed class SubeKapsamGetWriteTests(PostgresFixture fx)
{
    ...
    public async Task Operator_baska_sube_kirasini_okuyamaz_ve_iptal_edemez()
    {
        ...
        using (var op = host.ScopeFor(tenant, Guid.NewGuid(), "op", UserRole.Operator, assignedBranch: "Merkez"))
        {
            ...
            Assert.Contains("kapsamınız dışında", (await Assert.ThrowsAsync<NoPermissionException>(() => rentals.GetAsync(kira))).Message);
            ...
            Assert.Contains("OperationsDelete", (await Assert.ThrowsAsync<NoPermissionException>(() => rentals.CancelAsync(kira))).Message);
        }
