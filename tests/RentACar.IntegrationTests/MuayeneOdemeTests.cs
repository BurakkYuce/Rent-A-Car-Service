...
{
    private static async Task<(IServiceProvider sp, Guid inspId)> Seed(IServiceScope scope, string plate, decimal fee = 500m)
    {
        ...
        var insp = await sp.GetRequiredService<RegulationService>().AddInspectionAsync(
            v, new DateTimeOffset(2026, 1, 10, 0, 0, 0, TimeSpan.Zero), new DateTimeOffset(2028, 1, 10, 0, 0, 0, TimeSpan.Zero), fee);
        return (sp, insp);
    }
    ...
    public async Task Ode_ucret_arti_ceza_gider()
    {
        ...
        await reg.PayInspectionAsync(inspId, LedgerAccountType.Kasa, penalty: 100m);
        ...
        await Assert.ThrowsAsync<RentACar.Application.Common.ValidationException>(
            () => reg.PayInspectionAsync(inspId, LedgerAccountType.Kasa));
    }
    ...
    public async Task Donem_kilidi_engeller()
    {
        ...
        await Assert.ThrowsAsync<RentACar.Application.Common.ValidationException>(
            () => sp.GetRequiredService<RegulationService>().PayInspectionAsync(inspId, LedgerAccountType.Kasa));
    }
}
