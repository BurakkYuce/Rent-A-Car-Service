...
public sealed class AracGorunurlukTests(PostgresFixture fx)
{
    ...
    private static async Task SeedAsync(IServiceScope scope, params Vehicle[] araclar)
    {
    ...
    public async Task Tarih_araligi_secilen_tarih_tipine_uygulanir()
    {
        ...
        Assert.Equal(2, girisAralik.Total);
        Assert.Equal(["34TAR01", "34TAR02"], girisAralik.Items.Select(v => v.Plaka).Order().ToArray());
    ...
    public async Task Sube_filtresi_ve_operator_kapsami_birlikte_calisir()
    {
        ...
        using (var admin = host.ScopeFor(tenant))
        {
            ...
            var hepsi = admin.ServiceProvider.GetRequiredService<VehicleService>();
            Assert.Equal(2, (await hepsi.SearchAsync(new VehicleFilter { Sube = "Kadıköy", PageSize = 50 })).Total);
        }
    ...
    public async Task ListeEk_diger_tablolardan_cozulen_kolonlari_doldurur()
    {
        ...
        Assert.False(ek[a].AcikServis);
        Assert.True(ek[c].AcikBaf);
        Assert.True(ek[d].SatisVar);
        Assert.False(ek[f].SatisVar);              // iptal edilmiş satış "var" saymaz

        Assert.Equal(4200m, ek[e].KaskoPrim);      // trafik primi (999) DEĞİL
        Assert.Equal(Oca.AddYears(1), ek[e].KaskoBitis);
        Assert.Equal("Ziraat", ek[e].KrediKurulusu);
        Assert.Equal(Oca.AddMonths(12), ek[e].KrediSonTarih);  // 01.01.2026 + 12 taksit = 01.01.2027
    }
    ...
    public async Task ListeEk_tenant_izole()
    {
        ...
        using (var s1 = host.ScopeFor(t1))
        {
            ...
            db.Rentals.Add(new RentalContract
            {
                SozlesmeNo = "K-IZO", VehicleId = v.Id, MusteriId = cari.Id, Durum = RentalStatus.Kirada,
                BasTar = DateTimeOffset.UtcNow.AddDays(-1), BitTar = DateTimeOffset.UtcNow.AddDays(2)
            });
            await db.SaveChangesAsync();
            aracId = v.Id;
    ...
    public async Task Takvim_arac_kumesi_grup_ve_sube_filtresiyle_daralir()
    {
        ...
        var grupA = await svc.SearchAsync(new VehicleFilter { Grup = "A", PageSize = 200 });
        Assert.Equal(2, grupA.Total);
        ...
        var plaka = await svc.SearchAsync(new VehicleFilter { Query = "TKV05", PageSize = 200 });
        Assert.Equal("34TKV05", Assert.Single(plaka.Items).Plaka);
