...
public sealed class UiSecimMenuTests(WebFixture fx)
{
    ...
    public async Task Menu_rol_ve_izne_gore_suzulur()
    {
        ...
        foreach (var r in new[] { "/app/panel", "/app/kiralar/yeni", "/app/araclar", "/app/araclar/detayli", "/app/cariler", "/app/crm", "/app/hukuk", "/kasa", "/raporlar/karlilik", "/tarife-aktar", "/ayarlar", "/vade", "/bildirimler" })
        ...
        Assert.Equal(31, ogeler.Count(e => e.GetProperty("sahip").GetString() == "spa")); // F4.6: 3, F5.4: +8, F6.4: +14 (Araçlar 12 + Tanımlar 2), F7.3: +6 (Cariler & CRM)
        ...
        foreach (var r in new[] { "/app/araclar/detayli", "/app/musteri-taksit", "/vehicles", "/app/crm", "/crm", "/maliyet-hesapla", "/tarife-aktar", "/kasa", "/kurlar", "/raporlar/gunluk", "/ayarlar", "/subeler" })
            Assert.DoesNotContain(r, op);

        var (muh, _) = await MenuAsync(await GirisAsync(o, Kim.Muhasebe));
        foreach (var r in new[] { "/app/panel", "/kasa", "/faturalar", "/raporlar/gunluk", "/app/araclar/detayli", "/app/crm", "/maliyet-hesapla", "/app/musteri-taksit", "/vade" })
            Assert.Contains(r, muh);
        foreach (var r in new[] { "/app/kiralar/yeni", "/app/kiralar", "/app/araclar", "/vehicles", "/app/cariler", "/cariler", "/tarife-aktar", "/ayarlar" })
