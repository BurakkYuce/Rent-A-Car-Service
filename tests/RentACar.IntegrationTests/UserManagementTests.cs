...
public sealed class UserManagementTests(PostgresFixture fx)
{
    ...
    public async Task NonAdmin_cannot_manage_users()
    {
        ...
        await Assert.ThrowsAsync<NoPermissionException>(() => svc.CreateAsync(new UserInput
        { UserName = "x", Password = "gizli123", Rol = UserRole.Operator }));
        await Assert.ThrowsAsync<NoPermissionException>(() => svc.SetActiveAsync(Guid.NewGuid(), false));
    }
