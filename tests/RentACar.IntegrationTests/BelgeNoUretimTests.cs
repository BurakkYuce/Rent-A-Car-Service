...
public sealed class BelgeNoUretimTests(PostgresFixture fx)
{
    ...
    public async Task Tahsilat_ve_Tediye_AYRI_sayac()
    {
        ...
        await using var db = await CtxAsync(fx, tenant);
        await using var tx = await db.Database.BeginTransactionAsync();
        var th = await DocumentNoGenerator.GenerateAsync(db, tenant, DocumentNoType.Tahsilat, default);
    ...
    public async Task Rollback_BOSLUK_birakmaz()
    {
        ...
        await using (var tx = await db.Database.BeginTransactionAsync())
        {
            await DocumentNoGenerator.GenerateAsync(db, tenant, DocumentNoType.Gider, default);
    ...
    public async Task Eszamanli_tahsis_TEKRARSIZ()
    {
        ...
        var gorevler = Enumerable.Range(0, n).Select(async _ =>
        {
            await using var db = await CtxAsync(fx, tenant);
            await using var tx = await db.Database.BeginTransactionAsync();
            var no = await DocumentNoGenerator.GenerateAsync(db, tenant, DocumentNoType.Tahsilat, default);
            await tx.CommitAsync();
            return no;
        });
    ...
    public async Task Fatura_varsayilan_seri_ile_uretilir()
    {
        ...
        await using var db = await CtxAsync(fx, tenant);
        await using var tx = await db.Database.BeginTransactionAsync();
        var no = await DocumentNoGenerator.InvoiceAsync(db, tenant, default);
        await tx.CommitAsync();
    ...
    public async Task Fatura_sayaci_SERI_ve_YIL_basina()
    {
        ...
        await using (var tx = await db.Database.BeginTransactionAsync())
        {
            Assert.Equal("AAA2026000000001", await DocumentNoGenerator.InvoiceAsync(db, tenant, default, now: y1));
            Assert.Equal("AAA2026000000002", await DocumentNoGenerator.InvoiceAsync(db, tenant, default, now: y1));
            await tx.CommitAsync();
        }
        await using (var tx = await db.Database.BeginTransactionAsync())
        {
            // Yıl dönünce sıra 1'e döner — mevzuat şartı.
            Assert.Equal("AAA2027000000001", await DocumentNoGenerator.InvoiceAsync(db, tenant, default, now: y2));
            await tx.CommitAsync();
        }
    }
    ...
    public async Task Bicimsiz_seri_kodu_GURULTULU_reddedilir()
    {
        ...
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(
            () => DocumentNoGenerator.InvoiceAsync(db, tenant, default));
    ...
    public async Task Yeni_numaralar_eski_format_kayitlarla_CAKISMAZ()
    {
        ...
        await using var db = await CtxAsync(fx, tenant);
        await using var tx = await db.Database.BeginTransactionAsync();
        var no = await DocumentNoGenerator.GenerateAsync(db, tenant, DocumentNoType.AracSatis, default);
        await tx.CommitAsync();
