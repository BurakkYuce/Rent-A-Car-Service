...
public sealed class AracKrediTests(PostgresFixture fx)
{
    ...
    public async Task Kalan_bakiye_taksit_ode()
    {
        ...
        var svc = scope.ServiceProvider.GetRequiredService<VehicleLoanService>();
        ...
        DocumentNoOracle.OneOfExpected(15, 1, k0!.No);   // 15 = AracKredi
        var o0 = VehicleLoanService.Calculate(k0);
        ...
        for (var i = 0; i < 3; i++) Assert.True(await svc.PayInstallmentAsync(id));
        var o3 = VehicleLoanService.Calculate((await svc.GetAsync(id))!);
        Assert.Equal(90_000m, o3.KalanBakiye);        // 120000 − 3×10000 (elle oracle)

        for (var i = 0; i < 9; i++) await svc.PayInstallmentAsync(id);  // toplam 12
        var k12 = await svc.GetAsync(id);
        Assert.Equal(LoanStatus.Kapandi, k12!.Durum); // tüm taksitler ödendi
        Assert.Equal(0m, VehicleLoanService.Calculate(k12).KalanBakiye);
        Assert.False(await svc.PayInstallmentAsync(id));    // fazladan ödeme reddedilir
    }
    ...
    public async Task Faiz_toplam_geri_odeme()
    {
        ...
        var o = VehicleLoanService.Calculate((await svc.GetAsync(id))!);
    ...
    public async Task Tenant_isolation()
    {
        ...
        {
            await a.ServiceProvider.GetRequiredService<VehicleLoanService>()
        ...
        Assert.Empty(await b.ServiceProvider.GetRequiredService<VehicleLoanService>().ListAsync());
    }
}
