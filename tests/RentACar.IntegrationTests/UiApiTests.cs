...
public sealed class UiApiTests(WebFixture fx)
{
    ...
    private async Task<(HttpClient C, string Once, string Sonra)> GirisYap(
        string firma = "yucerent", string kullanici = "umit", WebFactory? f = null)
    {
        var c = (f ?? fx.Web).Istemci();
        var once = await XsrfAl(c);
        var r = await c.SendAsync(Istek(HttpMethod.Post, Giris, once, new { firma, kullanici, sifre = "umit1376" }));
    ...
    public async Task Giris_oturum_cerezi_ve_taze_xsrf_verir_ben_doner()
    {
        ...
        var once = await XsrfAl(c);
        var r = await c.SendAsync(Istek(HttpMethod.Post, Giris, once, new { firma = "yucerent", kullanici = "umit", sifre = "umit1376" }));
        ...
        var j = await Govde(r);
        Assert.Equal("umit", j.GetProperty("kullanici").GetProperty("kullaniciAdi").GetString());
        Assert.Equal("Ümit (Yüce Rent)", j.GetProperty("kullanici").GetProperty("adSoyad").GetString());
        Assert.Equal("yucerent", j.GetProperty("kiraci").GetProperty("kod").GetString());
        Assert.Equal("Yüce Rent A Car", j.GetProperty("kiraci").GetProperty("ad").GetString());
        Assert.Equal(fx.YucerentId, j.GetProperty("kiraci").GetProperty("id").GetGuid());
    ...
    {
        var (c, _, _) = await GirisYap(kullanici: "operator");
    ...
    public async Task Hatali_sifre_400_dogrulama_oturum_acilmaz()
    {
        ...
        var r = await c.SendAsync(Istek(HttpMethod.Post, Giris, t, new { firma = "yucerent", kullanici = "umit", sifre = "yanlis" }));
    ...
    public async Task Cikis_oturumu_kapatir_belirteci_yeniler()
    {
        ...
        var eski = await c.SendAsync(Istek(HttpMethod.Post, Giris, sonra, new { firma = "yucerent", kullanici = "umit", sifre = "umit1376" }));
        await ProblemBekle(eski, HttpStatusCode.BadRequest, "xsrf_gecersiz");
        var yeni = await c.SendAsync(Istek(HttpMethod.Post, Giris, cikisSonrasi, new { firma = "yucerent", kullanici = "umit", sifre = "umit1376" }));
    ...
    public async Task Giris_de_csrf_ister()
    {
        ...
        var r = await c.SendAsync(Istek(HttpMethod.Post, Giris, null, new { firma = "yucerent", kullanici = "umit", sifre = "umit1376" }));
    ...
    {
        var (_, _, umitBelirteci) = await GirisYap();
        var (op, _, _) = await GirisYap(kullanici: "operator");

        await ProblemBekle(await op.SendAsync(Istek(HttpMethod.Post, "/api/ui/v1/test/yaz", umitBelirteci)),
            HttpStatusCode.BadRequest, "xsrf_gecersiz");
    }
    ...
    {
        var (c, _, sonra) = await GirisYap(firma: "demo");
    ...
    {
        var yeni = await ApiSeed.TenantUserAsync(fx.Pg.OwnerConnectionString, "pilotac", "umit", "umit1376");
        await fx.PilotYapAsync(yeni, true);
        var (c, _, _) = await GirisYap(firma: "pilotac");
        Assert.Equal(HttpStatusCode.OK, (await c.GetAsync("/api/ui/v1/test/tamam")).StatusCode);
    ...
    {
        var id = await ApiSeed.TenantUserAsync(fx.Pg.OwnerConnectionString, "kapanacak", "umit", "umit1376");
        var (c, _, _) = await GirisYap(firma: "kapanacak");
    ...
    public async Task Platform_operatoru_ui_verisine_403_ben_401_yonlendirme_yok()
    {
        ...
        {
            ["kullanici"] = "admin", ["sifre"] = "platform1376",
        }));
    ...
    public async Task Giris_hiz_siniri_429_cok_istek_json()
    {
        ...
        var govde = new { firma = "yucerent", kullanici = "umit", sifre = "yanlis" };
    ...
    public async Task Blazor_yonlendirmeleri_degismedi()
    {
        ...
        {
            ["firma"] = "yucerent", ["kullanici"] = "umit", ["sifre"] = "umit1376",
        }));
        Assert.Equal(HttpStatusCode.Redirect, giris.StatusCode);
