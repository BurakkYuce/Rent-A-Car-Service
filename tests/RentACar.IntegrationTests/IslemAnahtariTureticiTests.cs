...
public sealed class IslemAnahtariTureticiTests
{
    ...
    private const string Header = "3f0e5a8c-9b1d-4c2e-8f7a-6d5c4b3a2910";
    ...
    [Theory]
    [InlineData(Header, "a4068cc6-1a4f-574a-a145-937a9b14eaac")]
    [InlineData("tahsilat-form-0001", "d8b25d16-e0a8-5c78-abf3-15a3ec210de4")]
    public void Test_vektorleri_python_ile_ayrica_hesaplandi(string title, string expected)
        => Assert.Equal(new Guid(expected), OperationKeyDeriver.Derive(T1, U1, title));

    [Fact]
    public void Ayni_baslik_farkli_kullanici_ya_da_kiraci_ASLA_ayni_anahtar_degil()
    {
        var k = OperationKeyDeriver.Derive(T1, U1, Header);
        Assert.Equal(new Guid("a9fdeecb-62a1-5b20-b5f7-b24d13338683"), OperationKeyDeriver.Derive(T1, U2, Header));
        Assert.Equal(new Guid("e63a9b3f-adb6-59b2-858f-2c8bd717ed75"), OperationKeyDeriver.Derive(T2, U1, Header));
        Assert.NotEqual(k, OperationKeyDeriver.Derive(T1, U2, Header));
        Assert.NotEqual(k, OperationKeyDeriver.Derive(T2, U1, Header));
        // Aynı üçlü → aynı anahtar (çift gönderim yakalanır).
        Assert.Equal(k, OperationKeyDeriver.Derive(T1, U1, Header));
    }
    ...
    public void Ham_istemci_degeri_anahtar_olmaz()
    {
        ...
        Assert.NotEqual(new Guid(Header), OperationKeyDeriver.Derive(T1, U1, Header));
    }

    [Fact]
    public void Surum_5_ve_RFC_varyanti()
    {
        var s = OperationKeyDeriver.Derive(T1, U1, Header).ToString("D");
    ...
    public void Gecersiz_baslik_reddedilir(string? value)
        => Assert.False(OperationKeyDeriver.IsValid(value));
    ...
    public void Turet_gecersiz_girdide_alanli_dogrulama_hatasi()
    {
        ...
        Assert.Throws<ArgumentException>(() => OperationKeyDeriver.Derive(Guid.Empty, U1, Header));
        Assert.Throws<ArgumentException>(() => OperationKeyDeriver.Derive(T1, Guid.Empty, Header));
    }
    ...
    private static DefaultHttpContext Ctx(Guid? tenant, Guid? user, params string[] headers)
    {
        ...
        if (headers.Length > 0) ctx.Request.Headers[IdempotencyHeader.Name] = headers;
        return ctx;
    }
    ...
    {
        Assert.Null(IdempotencyHeader.Key(Ctx(T1, U1)));
        Assert.Null(IdempotencyHeader.DeriveFromHeader(Ctx(null, null)));
    }
    ...
    public void Baslik_claimlerden_kiraci_ve_kullaniciyla_turetilir()
        => Assert.Equal(new Guid("a4068cc6-1a4f-574a-a145-937a9b14eaac"),
            IdempotencyHeader.Key(Ctx(T1, U1, Header)));

    [Fact]
    public void Deterministik_anahtar_varsa_baslik_ezemez()
    {
        var collectionKey = new Guid("cccccccc-0000-0000-0000-000000000003");
        Assert.Equal(collectionKey, IdempotencyHeader.Key(Ctx(T1, U1, Header), collectionKey));
    }

    [Fact]
    public void Bicimsiz_ya_da_cok_degerli_baslik_400()
    {
        var ex = Assert.Throws<ValidationException>(() => IdempotencyHeader.Key(Ctx(T1, U1, "kisa")));
        Assert.Equal(IdempotencyHeader.Name, ex.Alan);
        Assert.Throws<ValidationException>(() => IdempotencyHeader.Key(Ctx(T1, U1, "")));
        Assert.Throws<ValidationException>(() => IdempotencyHeader.Key(Ctx(T1, U1, Header, Header + "-2")));
        // Deterministik anahtar olsa bile bozuk başlık sessizce yutulmaz.
        Assert.Throws<ValidationException>(() => IdempotencyHeader.Key(Ctx(T1, U1, "kisa"), Guid.NewGuid()));
    }

    [Fact]
    public void Zorunlu_anahtar_basliksiz_ve_deterministiksiz_istekte_400()
    {
        var ex = Assert.Throws<ValidationException>(() => IdempotencyHeader.RequiredKey(Ctx(T1, U1)));
        Assert.Equal(IdempotencyHeader.Name, ex.Alan);
        Assert.Throws<ValidationException>(() => IdempotencyHeader.RequiredKey(Ctx(T1, U1), Guid.Empty));
        var det = new Guid("dddddddd-0000-0000-0000-000000000004");
        Assert.Equal(det, IdempotencyHeader.RequiredKey(Ctx(T1, U1), det));
        Assert.Equal(new Guid("a4068cc6-1a4f-574a-a145-937a9b14eaac"), IdempotencyHeader.RequiredKey(Ctx(T1, U1, Header)));
    }

    [Fact]
    public void Kimliksiz_istekte_turetme_yapilmaz()
        => Assert.Throws<InvalidOperationException>(() => IdempotencyHeader.Key(Ctx(null, null, Header)));
}
