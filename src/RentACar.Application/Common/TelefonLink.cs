...
/// bozulmasın.</para>
/// </summary>
public static class TelefonLink
{
    ...
    public static string? Wa(string? tel, string? mesaj = null)
    {
        if (Normalize(tel) is not { } d) return null;
        var url = "https://wa.me/" + d;
        return string.IsNullOrWhiteSpace(mesaj) ? url : url + "?text=" + Uri.EscapeDataString(mesaj);
    }
}
