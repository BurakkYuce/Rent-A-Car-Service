...
/// yazmadan yeni uca bağlamak için <see cref="PagingExtensions.ToPage{T}(PagedResult{T})"/> köprüsü var.
...
public sealed record Sayfa<T>(IReadOnlyList<T> Kayitlar, int Toplam, int SayfaNo, int Boyut)
{
    ...
    public Sayfa<TSonuc> Convert<TSonuc>(Func<T, TSonuc> convert)
        => new(Kayitlar.Select(convert).ToList(), Toplam, SayfaNo, Boyut);
}
...
public static class PagingExtensions
{
    ...
    public static Sayfa<T> ToPage<T>(this PagedResult<T> result)
