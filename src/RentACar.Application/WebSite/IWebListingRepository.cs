...
public interface IWebListingRepository
{
    ...
    Task<IReadOnlyList<string>> ListSlugsAsync(CancellationToken ct = default);
    ...
    Task<IReadOnlyList<WebIlan>> FindByKeyAsync(string key, CancellationToken ct = default);
    ...
    ///
    /// <paramref name="groups"/>'daki <c>Yeni=false</c> satırlar MEVCUT bir ilana katılımdır —
    /// ilan INSERT EDİLMEZ, yalnız araç üyelikleri yazılır (ikiz-ilan koruması bu yolu kullanır).
    /// </summary>
    Task CreateWithMembershipAsync(IReadOnlyList<(WebIlan Ilan, bool Yeni, IReadOnlyList<Guid> AracIdler)> groups,
    ...
    /// <summary>F11.1b — satır kilidi + sürüm karşılaştırması; uyuşmazlık <see cref="Common.ConcurrentModificationException"/>.</summary>
    ...
    Task<int> CopyPriceToSiblingsAsync(Guid sourceListingId, CancellationToken ct = default);

    /// <summary>Adım-3: özellik satırlarını TAMAMEN değiştirir (sil + yaz, tek transaction).</summary>
    Task ReplaceFeaturesAsync(Guid listingId, IReadOnlyList<WebIlanOzellik> rows, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<Vehicle>> ListVehiclesWithoutListingAsync(CancellationToken ct = default);
}
