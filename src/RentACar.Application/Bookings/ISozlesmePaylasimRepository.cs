...
/// </summary>
public interface ISozlesmePaylasimRepository
{
    /// <summary>Aktif (iptal edilmemiş) link — yoksa <c>null</c>.</summary>
    Task<PaylasimDurum?> AktifAsync(Guid rentalId, CancellationToken ct = default);
    ...
    /// </summary>
    Task<PaylasimDurum> OlusturAsync(Guid rentalId, string sozlesmeNo, string token, byte[] pdf,
    ...
    /// </summary>
    Task<PaylasimDurum> YeniSurumAsync(Guid rentalId, string sozlesmeNo, string token, byte[] pdf,
        CancellationToken ct = default);
    ...
    /// </summary>
    Task<bool> IptalAsync(Guid rentalId, CancellationToken ct = default);
}
