...
/// <see cref="IPeriodClosingRepository"/>'ye devreder. Kapanış Gelir/Gider bakiyelerini sıfırlayıp net dönem
...
/// </summary>
public sealed class PeriodClosingVoucherService(
    IReportRepository reports, IPeriodClosingRepository repo, PeriodLockService period,
    ICurrentUser currentUser, ScreenPermissionService screens)
{
    ...
    public async Task<DonemKapanisOnizleme> PreviewAsync(DateTimeOffset closingDate, CancellationToken ct = default)
    {
        var (revenueSigned, expenseSigned) = await ReadBalanceAsync(ClosingInstant(closingDate), ct);
        // P&L: gelir = −Σ SignedBase(Gelir) (gelir Alacak → negatif signed); gider = +Σ SignedBase(Gider).
        var revenue = -revenueSigned;
        var expense = expenseSigned;
        return new DonemKapanisOnizleme(revenue, expense, revenue - expense);
    }
    ...
    public async Task CloseAsync(DateTimeOffset closingDate, CancellationToken ct = default)
    {
        ...
        var existing = await period.GetClosingDateAsync(ct);
        if (existing is { } m && PeriodLock.LocalDay(closingDate) <= PeriodLock.LocalDay(m))
        ...
        await repo.CloseAsync(closingDate, ct);
    }

    /// <summary>Kapanış anına (dahil) kadarki Gelir/Gider GÜNCEL SignedBase bakiyeleri (önceki kapanışlar DAHİL).</summary>
    private async Task<(decimal gelirSigned, decimal giderSigned)> ReadBalanceAsync(DateTimeOffset closingInstant, CancellationToken ct)
    {
        var rows = await reports.GetLedgerRowsAsync(
            [LedgerAccountType.Gelir, LedgerAccountType.Gider], null, closingInstant, ct);
    ...
    private static DateTimeOffset ClosingInstant(DateTimeOffset closingDate)
        => PeriodLock.DayEndUtc(PeriodLock.LocalDay(closingDate));
}
