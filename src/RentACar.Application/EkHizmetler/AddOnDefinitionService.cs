...
/// </summary>
public sealed class AddOnDefinitionService(IAddOnDefinitionRepository repository, ICurrentUser currentUser, ITenantCache cache,
    IRowVersionStore? rowVersions = null)
{
    private readonly IAddOnDefinitionRepository _repository = repository;
    ...
    public async Task<bool> UpdateAsync(Guid id, EkHizmetTanimInput input, CancellationToken ct = default)
    {
        ...
        if (existing is not null && (SystemCode(existing.Kod) || SystemCode(n.Kod))
            && !string.Equals(existing.Kod, n.Kod, StringComparison.OrdinalIgnoreCase))
    ...
    public async Task<bool> UpdateVersionedAsync(Guid id, EkHizmetTanimInput input, string expectedVersion, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                if ((SystemCode(t.Kod) || SystemCode(n.Kod)) && !string.Equals(t.Kod, n.Kod, StringComparison.OrdinalIgnoreCase))
                    throw new ValidationException("Sistem ücret tanımının (SYS-*) kodu değiştirilemez; normal tanım SYS- önekini alamaz.");
                Apply(t, n);
                t.UpdatedAtUtc = DateTimeOffset.UtcNow;
    ...
    private static bool SystemCode(string? code) => code?.StartsWith("SYS-", StringComparison.OrdinalIgnoreCase) == true;
