...
public sealed class PersonnelService(
    IPersonnelRepository repository, ICurrentUser currentUser, ISecretProtector secrets, ScreenPermissionService screens)
{
    ...
    /// </summary>
    public async Task<IReadOnlyList<Personel>> SearchAsync(PersonelFilter? filter = null, CancellationToken ct = default)
    {
        var all = await ListAsync(ct);
        var f = filter ?? new PersonelFilter();

        IEnumerable<Personel> q = all;
        ...
        {
            var t = f.Ara.Trim();
            q = q.Where(p => Contains(p.Kod, t) || Contains(p.Ad, t) || Contains(p.Soyad, t)
                          || Contains($"{p.Ad} {p.Soyad}", t) || Contains(p.CepTel, t) || Contains(p.MailAdresi, t));
        }
    ...
    private static bool Contains(string? source, string term)
        => source is not null && source.Contains(term, StringComparison.CurrentCultureIgnoreCase);
    ...
    public async Task<PersonelDetail?> GetDetailAsync(Guid id, CancellationToken ct = default)
    {
        ...
        return new PersonelDetail(
            ...
            r.IseGiris, r.IseCikis, r.SurucuBelgeNo, ParseSalary(_secrets.Unprotect(r.MaasEnc)), r.Sube, r.Aktif,
            Ham: r);
    }
    ...
    public async Task<Guid> CreateAsync(PersonelInput input, CancellationToken ct = default)
    {
        ...
        row.MaasEnc = _secrets.Protect(SalaryToText(n.Maas));
    ...
    private void ApplyUpdate(Personel row, PersonelInput n)
    {
        ...
        if (n.Maas is not null) row.MaasEnc = _secrets.Protect(SalaryToText(n.Maas));
    ...
    private static string? SalaryToText(decimal? m) => m?.ToString(CultureInfo.InvariantCulture);
    private static decimal? ParseSalary(string? s)
