...
/// (<see cref="LinkAsync"/>) + giderleştirme (<see cref="ConvertToExpenseAsync"/>).
...
/// icat ETMEZ. Tek para yolu <see cref="ConvertToExpenseAsync"/>'tir ve o da işi olduğu gibi
...
/// </summary>
public sealed class IncomingEInvoiceService(
    IIncomingEInvoiceRepository repository, IEInvoiceService einvoice, ICurrentUser currentUser,
    ExpenseService expenses)
{
    private readonly IIncomingEInvoiceRepository _repository = repository;
    ...
    public async Task<Guid> CreateManualAsync(GelenEFaturaInput input, CancellationToken ct = default)
    {
        ...
        IncomingEInvoiceVatBreakdown.ValidateTotals(input.NetTutar, input.KdvTutar, input.GenelToplam);
        ...
        var row = new GelenEFatura
        {
            ...
            Durum = IncomingEInvoiceStatus.Beklemede
        };
    ...
    public Task<bool> ApproveAsync(Guid id, CancellationToken ct = default)
        => TransitionAsync(id, IncomingEInvoiceStatus.Beklemede, IncomingEInvoiceStatus.Onaylandi,
            "Yalnız beklemedeki fatura onaylanabilir.", null, ct);

    public Task<bool> RejectAsync(Guid id, string? reason, CancellationToken ct = default)
        => TransitionAsync(id, IncomingEInvoiceStatus.Beklemede, IncomingEInvoiceStatus.Reddedildi,
            "Yalnız beklemedeki fatura reddedilebilir.", string.IsNullOrWhiteSpace(reason) ? null : reason.Trim(), ct);

    public Task<bool> IsleAsync(Guid id, CancellationToken ct = default)
        => TransitionAsync(id, IncomingEInvoiceStatus.Onaylandi, IncomingEInvoiceStatus.Islendi,
            "Yalnız onaylanmış fatura işlenebilir.", null, ct);

    private async Task<bool> TransitionAsync(
        Guid id, IncomingEInvoiceStatus from, IncomingEInvoiceStatus to, string error, string? rejectionReason, CancellationToken ct)
    {
        PermissionGuard.Require(_currentUser, Permission.FinanceWrite);
        ...
        {
            if (row.Durum != from) throw new ValidationException(error);
            row.Durum = to;
            if (rejectionReason is not null) row.RedNedeni = rejectionReason;
    ...
    /// bağını kaydeder. Hepsi BİLGİDİR — deftere YAZMAZ; yalnız <see cref="ConvertToExpenseAsync"/>'e girdi olur.
    ///
        ...
        GelenEFaturaBaglamaInput input, CancellationToken ct = default, string? expectedVersion = null)
    {
        ...
        return await _repository.UpdateLockedAsync(input.Id, expectedVersion, row =>
        {
            ...
            if (row.Durum == IncomingEInvoiceStatus.Reddedildi)
            ...
            IncomingEInvoiceVatBreakdown.ValidateTotals(row.NetTutar, row.KdvTutar, row.GenelToplam);
            IncomingEInvoiceVatBreakdown.Validate(row);
        }, ct);
    }
    ...
    ///
    /// <para><b>Kuruş:</b> yuvarlama SATIR BAZINDA yapılır; <see cref="IncomingEInvoiceVatBreakdown.Resolve"/>
    ...
    ///
    /// <para><b>Bilinen dar yarış (Low, bilinçli):</b> <see cref="LinkAsync"/> tam olarak
    ...
    /// </summary>
    public async Task<int> ConvertToExpenseAsync(GelenEFaturaGiderInput input, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.FinanceWrite);
        ...
        if (row.GiderlestirilmeUtc is not null)
            throw new DuplicateOperationException($"'{row.Ettn}' ETTN'li fatura zaten giderleştirilmiş.");
        ...
        if (row.Durum != IncomingEInvoiceStatus.Onaylandi && !resumable)
        ...
        await _repository.UpdateLockedAsync(row.Id, null, r =>
        {
            if (r.GiderlestirilmeUtc is not null)
                throw new DuplicateOperationException($"'{r.Ettn}' ETTN'li fatura zaten giderleştirilmiş.");
            if (r.GiderIslemAnahtari != r.Id)
            {
                if (r.Durum != IncomingEInvoiceStatus.Onaylandi)
                    throw new ValidationException("Yalnız onaylanmış gelen fatura giderleştirilebilir.");
                r.Durum = IncomingEInvoiceStatus.Islendi;
        ...
        var items = BuildExpenseLines(claimed!, input);
        try
        {
            // TEK transaction + deterministik idempotency anahtarı (batch = faturanın Id'si).
            await _expenses.BatchCreateAsync(items, batchKey: row.Id, ct);
        }
        ...
        catch (ValidationException) when (claimedNow)
        {
            ...
            await _repository.UpdateLockedAsync(row.Id, null, r =>
            {
                ...
                r.Durum = IncomingEInvoiceStatus.Onaylandi;
    ...
    {
        var rows = IncomingEInvoiceVatBreakdown.Resolve(row); // kuruş-birebir doğrulama + oran çözümü

        var customerId = input.CariId ?? row.CariId;
        if (input.OdemeYontemi == PaymentMethod.AcikHesap && (customerId is null || customerId == Guid.Empty))
        ...
        return rows.Select(s => new ExpenseInput
        {
            Tip = tip,
            Tarih = row.Tarih,
            VehicleId = row.VehicleId,
            CariId = input.OdemeYontemi == PaymentMethod.AcikHesap ? customerId : null,
            Sube = input.Sube,
            EvrakNo = Clamp(row.Ettn, 64),
            NetTutar = s.Matrah,
            KdvOrani = s.Oran,
            Doviz = row.Currency,
            ...
            Aciklama = Clamp(string.Format(System.Globalization.CultureInfo.InvariantCulture,
