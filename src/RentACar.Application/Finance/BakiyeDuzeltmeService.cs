...
public enum BakiyeDuzeltmeYonu
{
...
public sealed class BakiyeDuzeltmeInput
{
    ...
    public BakiyeDuzeltmeYonu Yon { get; set; }
...
/// </summary>
public sealed class BakiyeDuzeltmeService(
    ILedgerPoster ledger, ICustomerRepository customers, ICurrentUser currentUser,
    IPeriodLockGuard periodLock, RentACar.Application.Kur.KurCozucu kurCozucu)
{
    ...
    public const string Kaynak = "BakiyeDuzeltme";
    ...
    public async Task<Guid> AdjustAsync(BakiyeDuzeltmeInput input, CancellationToken ct = default)
    {
        ...
        TarihPolitikasi.ParaTarihi(input.Tarih, "Bakiye düzeltme");
        ...
        await _lock.EnsureOpenAsync(tarih, ct);   // kapalı döneme düzeltme YOK
        ...
        var money = new Money(input.Tutar, Kur.KurService.NormalizeKodStrict(input.Doviz), cozulenKur);

        var sourceId = input.IslemAnahtari is { } k && k != Guid.Empty ? k : Guid.NewGuid();
        var desc = string.IsNullOrWhiteSpace(input.Aciklama)
            ? (input.Yon == BakiyeDuzeltmeYonu.Alacaklandir ? "Bakiye düzeltme (alacaklandırma)" : "Bakiye düzeltme (borçlandırma)")
            : input.Aciklama.Trim();
        ...
        var cariYon = input.Yon == BakiyeDuzeltmeYonu.Alacaklandir
            ? LedgerDirection.Credit    // bakiye düşer
            : LedgerDirection.Debit;    // bakiye artar
        var karsiYon = cariYon == LedgerDirection.Debit ? LedgerDirection.Credit : LedgerDirection.Debit;

        await _ledger.PostAsync(
        [
            new AccountLedgerEntry
            {
                EntryDateUtc = tarih, AccountType = LedgerAccountType.Cari, AccountRef = input.CariId,
                Direction = cariYon, Amount = money, SourceType = Kaynak, SourceId = sourceId, Description = desc
            },
            new AccountLedgerEntry
            {
                ...
                EntryDateUtc = tarih, AccountType = LedgerAccountType.MuhasebeDuzeltmesi, AccountRef = null,
                Direction = karsiYon, Amount = money, SourceType = Kaynak, SourceId = sourceId, Description = desc
            }
        ], ct);
        return sourceId;
    }
