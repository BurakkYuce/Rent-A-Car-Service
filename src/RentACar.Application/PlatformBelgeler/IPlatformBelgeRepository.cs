...
/// </summary>
public interface IPlatformBelgeRepository
{
    /// <summary>Bu tenant'ın görebileceği YAYINDA belgeler. <paramref name="yoneticiMi"/> false ise
    /// <c>YalnizYoneticiler</c> belgeler DIŞARIDA bırakılır. Blob SEÇİLMEZ.</summary>
    Task<IReadOnlyList<FirmaBelgeSatiri>> ListeleAsync(Guid tenantId, bool yoneticiMi, CancellationToken ct = default);
    ...
    /// <paramref name="yoneticiMi"/>). Aksi halde <c>null</c> — "yok" ile "yetkisiz" AYIRT EDİLMEZ
    /// (varlık bilgisi de sızmasın).
    /// </summary>
    Task<BelgeIcerik?> IndirAsync(Guid belgeId, Guid tenantId, bool yoneticiMi, CancellationToken ct = default);
}
