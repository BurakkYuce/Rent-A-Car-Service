...
/// </summary>
public sealed class PlatformBelgeService(IPlatformBelgeRepository repository, ICurrentUser currentUser, ITenantContext tenant)
{
    ...
    private bool YoneticiMi => currentUser.Role is UserRole.Admin or UserRole.Yonetici;

    public Task<IReadOnlyList<FirmaBelgeSatiri>> ListeleAsync(CancellationToken ct = default)
        => repository.ListeleAsync(TenantIdOrThrow, YoneticiMi, ct);
    ...
    public Task<BelgeIcerik?> IndirAsync(Guid belgeId, CancellationToken ct = default)
        => repository.IndirAsync(belgeId, TenantIdOrThrow, YoneticiMi, ct);
}
