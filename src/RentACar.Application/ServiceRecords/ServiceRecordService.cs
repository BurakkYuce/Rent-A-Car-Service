...
/// <see cref="ReflectAsync"/> rücusudur ve o da <c>ToplamIscilik</c> (Σ kalem NET) üzerinden gider —
...
public sealed class ServiceRecordService(
    ...
    RentACar.Application.Kur.ExchangeRateResolver exchangeRateResolver, IRowVersionStore? rowVersions = null)
{
    ...
    public async Task<Guid> CreateAsync(ServiceRecordInput input, CancellationToken ct = default)
    {
        ...
        ValidateInfo(input);
        var items = input.Lines.Select(PrepareItem).ToList();
        ...
        var record = new ServiceRecord
        {
            ...
            Durum = input.Rezervasyon ? ServiceStatus.Rezerve : ServiceStatus.Acik,
    ...
    /// </summary>
    public Task<bool> UpdateInfoAsync(Guid id, ServiceRecordBilgiInput input, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.OperationsWrite);
        ValidateInfo(input);
        return _repository.UpdateInfoAsync(id, r => ApplyInfo(r, input), ct);
    }
    ...
    /// <summary>F9.1 — <see cref="UpdateInfoAsync"/> under a row lock with a version check (409 <c>cakisma</c>).
    /// Same whitelist: only the FAZ-16 information blocks; no ledger effect.</summary>
    public Task<bool> UpdateInfoVersionedAsync(Guid id, ServiceRecordBilgiInput input, string expectedVersion,
    ...
    /// </summary>
    public Task<bool> TakeIntoServiceAsync(Guid id, int? entryKm = null, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.OperationsWrite);
        if (entryKm is < 0) throw new ValidationException("Giriş KM negatif olamaz.");
        return _repository.TransitionAsync(id, r =>
        {
            if (r.Durum != ServiceStatus.Rezerve)
                throw new ValidationException("Yalnız 'Rezerve' kayıt servise alınabilir.");
            r.Durum = ServiceStatus.Acik;
            r.GirisTarihi = DateTimeOffset.UtcNow;
            if (entryKm is int km) r.GirisKm = km;
    ...
    public Task<bool> CompleteAsync(Guid id, int pickupKm, int? nextMaintenanceKm = null, CancellationToken ct = default)
        ...
        { VehicleId = r.VehicleId, Tarih = r.CikisTarihi!.Value, Km = pickupKm, Kaynak = KmLogSource.Servis },
        ct: ct);
    ...
    public Task<bool> CancelAsync(Guid id, CancellationToken ct = default)
    {
        ...
        return _repository.TransitionAsync(id, r =>
        {
            ...
            if (r.Durum is ServiceStatus.Tamamlandi or ServiceStatus.Iptal)
                throw new ValidationException("Kapanmış servis iptal edilemez.");
            r.Durum = ServiceStatus.Iptal;
    ...
    public Task<bool> AddItemAsync(Guid id, string description, decimal amount, CancellationToken ct = default)
        => AddItemAsync(id, new ServiceLineInput { Aciklama = description, Tutar = amount }, ct);
    ...
    /// </summary>
    public Task<bool> AddItemAsync(Guid id, ServiceLineInput item, CancellationToken ct = default)
        => _repository.AddLineAsync(id, PrepareItem(item), ct);
    ...
    /// </summary>
    public async Task ReflectAsync(Guid serviceId, Guid customerId, DateTimeOffset? date = null,
        string? currency = "TRY", decimal? exchangeRate = null, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.FinanceWrite);
        if (customerId == Guid.Empty) throw new ValidationException("Yansıtılacak cari seçilmelidir.");
        ...
        if (rec.Durum != ServiceStatus.Tamamlandi) throw new ValidationException("Yalnız tamamlanmış servis yansıtılabilir.");
        if (rec.HasarSorumlu is not (DamageResponsible.Musteri or DamageResponsible.Sigorta))
        ...
        var reflected = decimal.Round(rec.ToplamIscilik * rec.KusurOrani.Value, 2, MidpointRounding.AwayFromZero);
        if (reflected <= 0m) throw new ValidationException("Yansıtılacak tutar pozitif olmalıdır.");
        ...
        var resolvedRate = await _exchangeRateResolver.ResolveAsync(currency, exchangeRate, entryDate, ct);
        var money = new Money(reflected, (currency ?? "TRY").Trim().ToUpperInvariant(), resolvedRate);
        var desc = $"Servis rücu {rec.No} (kusur %{rec.KusurOrani.Value * 100m:0.##})";
        await _repository.PostReflectionAsync(serviceId, customerId, reflected,
        [
            new AccountLedgerEntry { EntryDateUtc = entryDate, AccountType = LedgerAccountType.Cari, AccountRef = customerId,
    ...
    private static string? Clamp(string? s) => string.IsNullOrWhiteSpace(s) ? null : s.Trim();

    /// <summary>Kalem girdisini doğrular ve kalıcı satıra çevirir (tutar gerekiyorsa türetilir).</summary>
    private static ServiceLine PrepareItem(ServiceLineInput l)
    {
        ...
        var quantity = l.BirimFiyat is null ? l.Miktar : l.Miktar ?? 1m;

        var amount = l.Tutar ?? (l.BirimFiyat is { } bf
            ? ServiceItemCalculation.Net(bf, quantity, l.Indirim)
        ...
        {
            Aciklama = l.Aciklama.Trim(), Tutar = amount,
            BirimFiyat = l.BirimFiyat, Miktar = quantity, Indirim = l.Indirim, KdvOran = l.KdvOran
        };
    ...
    private static void ValidateInfo(ServiceRecordBilgiInput b)
    {
        ...
        if (Clamp(b.OdemeDoviz) is { Length: not 3 })
        ...
        DatePolicy.MoneyDate(b.KazaTarihi, "Kaza");
        DatePolicy.MoneyDate(b.FaturaTarihi, "Fatura");
        DatePolicy.MoneyDate(b.OdemeTarihi, "Ödeme");
    ...
    private static void ApplyInfo(ServiceRecord r, ServiceRecordBilgiInput b)
    {
        r.AtolyeAdi = Clamp(b.AtolyeAdi);
        r.Aciklama = Clamp(b.Aciklama);

        r.BeyanTuru = Clamp(b.BeyanTuru);
        r.KarsiPlaka = Clamp(b.KarsiPlaka)?.ToUpperInvariant();
        r.KarsiTrafikSigortasi = Clamp(b.KarsiTrafikSigortasi);
        r.KazaTarihi = b.KazaTarihi;
        r.KazaSorumlusu = Clamp(b.KazaSorumlusu);
        r.HasarDosyaNo = Clamp(b.HasarDosyaNo);
        r.DegerKaybi = b.DegerKaybi;

        r.FaturaTarihi = b.FaturaTarihi;
        r.FaturaNo = Clamp(b.FaturaNo);
        ...
        r.FaturaGenelToplam = b.FaturaTutar is null && b.FaturaKdv is null
            ? null
            : ServiceItemCalculation.Round((b.FaturaTutar ?? 0m) + (b.FaturaKdv ?? 0m));

        r.OdemeTarihi = b.OdemeTarihi;
        r.Odeme = b.Odeme;
        r.OdemeDoviz = Clamp(b.OdemeDoviz)?.ToUpperInvariant();
        r.OdemeKur = b.OdemeKur;
        r.OdemeTuru = b.OdemeTuru;
        r.KasaKodu = Clamp(b.KasaKodu);
        r.HesapNo = Clamp(b.HesapNo);
