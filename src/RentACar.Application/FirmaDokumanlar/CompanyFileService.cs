...
/// (1) tenant başına en fazla <see cref="MaxDocuments"/> belge, (2) dosya ≤ 3 MB, (3) yalnız PDF.
...
/// </summary>
public sealed class CompanyFileService(ICompanyFileRepository repository, ICurrentUser currentUser)
{
    /// <summary>Tenant başına belge üst sınırı (ürün kararı). Yuva numaraları 1..10.</summary>
    public const int MaxDocuments = 10;

    /// <summary>Dosya üst sınırı — <see cref="PdfValidation.MaxBytes"/> (3 MB) ile TEK kaynaktan.</summary>
    public const long MaxBytes = PdfValidation.MaxBytes;

    // ---- Okuma: guard YOK (oturum açmış herkes) ----

    public Task<IReadOnlyList<FirmaDokumanSatiri>> ListAsync(CancellationToken ct = default)
    ...
    public Task<FirmaDokumanIcerik?> DownloadAsync(Guid id, CancellationToken ct = default)
        => repository.DownloadAsync(id, ct);

    // ---- Yazma: OperationsWrite ----

    public async Task<Guid> UploadAsync(FirmaDokumanInput input, CancellationToken ct = default)
    {
        PermissionGuard.Require(currentUser, Permission.OperationsWrite);

        var title = (input.Baslik ?? string.Empty).Trim();
        if (title.Length == 0) throw new ValidationException("Başlık zorunludur.");
        ...
        if (await repository.SayAsync(ct) >= MaxDocuments)
            throw new ValidationException(
                $"En fazla {MaxDocuments} doküman saklanabilir. Yeni belge yüklemek için mevcutlardan birini silin.");

        var document = new FirmaDokuman
        {
            Baslik = title,
            Aciklama = string.IsNullOrWhiteSpace(input.Aciklama) ? null : input.Aciklama.Trim(),
            DosyaAdi = PdfValidation.SafeFileName(input.DosyaAdi ?? title),
            Bytes = bytes,
        ...
        return await repository.AddAsync(document, MaxDocuments, ct);
    }
