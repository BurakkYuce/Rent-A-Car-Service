...
/// </summary>
public sealed class FormVarsayilanCozucu(ITenantSettingsRepository ayarlar)
{
    ...
    public const int YakitVarsayilan = 8;

    /// <summary>Tenant'ın çıkış yakıt varsayılanı; yoksa <see cref="YakitVarsayilan"/>.
    /// Aralık dışı (eski/bozuk) kayıt da sabite düşer — teslim formunun HTML min/max'ıyla çakışmasın.</summary>
    public async Task<int> CikisYakitAsync(CancellationToken ct = default)
    {
        var v = (await ayarlar.GetAsync(ct))?.VarsayilanYakitSeviyesi;
        return v is >= 0 and <= 12 ? v.Value : YakitVarsayilan;
    }
    ...
    /// </summary>
    public async Task<string?> FiyatTuruAsync(CancellationToken ct = default)
        => Pricing.FiyatTuruSecenek.Normalize((await ayarlar.GetAsync(ct))?.VarsayilanFiyatTuru);
}
