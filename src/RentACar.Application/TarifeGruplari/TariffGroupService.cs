...
/// </summary>
public sealed class TariffGroupService(
    ITariffGroupRepository repository, ICurrentUser currentUser, IPasswordHasher hasher,
    IRowVersionStore? rowVersions = null)
{
    private readonly ITariffGroupRepository _repository = repository;
    ...
    public async Task<Guid> CreateAsync(TarifeGrubuInput input, CancellationToken ct = default)
    {
        ...
        Validate(n);
        if (await _repository.CodeExistsAsync(n.Kod, null, ct))
    ...
    public async Task<bool> UpdateAsync(Guid id, TarifeGrubuInput input, CancellationToken ct = default)
    {
        ...
        var n = Normalize(input);
        Validate(n);
        if (await _repository.CodeExistsAsync(n.Kod, id, ct))
            throw new ValidationException($"'{n.Kod}' kodlu tarife grubu zaten var.");
