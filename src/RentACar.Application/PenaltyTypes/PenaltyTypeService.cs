...
public sealed class PenaltyTypeService(IPenaltyTypeRepository repository, ICurrentUser currentUser)
{
    ...
    public Task<bool> UpdateAsync(Guid id, PenaltyTypeInput input, CancellationToken ct = default)
    ...
    public async Task<bool> UpdateAsync(Guid id, PenaltyTypeInput input, string? expectedVersion, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.OperationsWrite);
        var n = Normalize(input);
        Validate(n);
