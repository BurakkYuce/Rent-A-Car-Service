...
/// </summary>
public sealed class RezSartService(
    IRezSartRepository repository, ICustomerRepository customers, ICurrentUser currentUser)
{
    private readonly IRezSartRepository _repository = repository;
    ...
    public async Task<bool> KarsilandiIsaretleAsync(Guid id, string? teslimEden = null, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.OperationsWrite);
        return await _repository.UpdateAsync(id, row =>
        {
            row.KarsilamaTarihi ??= DateTimeOffset.UtcNow;
            var t = TrimOrNull(teslimEden);
    ...
    public async Task<bool> KarsilamaGeriAlAsync(Guid id, CancellationToken ct = default)
    {
        PermissionGuard.Require(_currentUser, Permission.OperationsWrite);
        return await _repository.UpdateAsync(id, row =>
        {
    ...
    private async Task MusteriVarMi(Guid musteriId, CancellationToken ct)
    {
        // Tenant filtresi repo katmanında → başka tenant'ın müşterisi "bulunamadı" döner.
        if (await _customers.FindAsync(musteriId, ct) is null)
    ...
    private static void Validate(RezSartInput n)
    {
        ...
        var sinir = DateTimeOffset.UtcNow.AddDays(1);
        if (n.TalepTarihi is { } tal && tal > sinir)
            throw new ValidationException("Talep tarihi gelecekte olamaz.");
        if (n.KarsilamaTarihi is { } kar && kar > sinir)
