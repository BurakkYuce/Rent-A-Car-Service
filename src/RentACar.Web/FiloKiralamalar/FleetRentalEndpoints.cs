...
public static class FleetRentalEndpoints
{
    public static IEndpointRouteBuilder MapFleetRentalEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        {
            try { await svc.CancelAsync(id); return Result.Ok("/filo-kiralama", "İşlem iptal edildi."); }
            catch (ValidationException ex) { return Results.Redirect($"/filo-kiralama?hata={Uri.EscapeDataString(ex.Message)}"); }
        }).RequirePermission(Permission.OperationsDelete);

        grp.MapPost("/tamamla", async (FleetRentalService svc, [FromForm] Guid id) =>
        {
            try { await svc.CompleteAsync(id); return Result.Ok("/filo-kiralama", "Tamamlandı."); }
            catch (ValidationException ex) { return Results.Redirect($"/filo-kiralama?hata={Uri.EscapeDataString(ex.Message)}"); }
        });
