...
public static partial class WebsiteApi
{
    ...
    private static void MapListings(RouteGroupBuilder v1)
    {
        ...
        g.MapGet("/ozet", async (WebIlanService s, CancellationToken ct) =>
        {
            var rows = await s.ListAsync(ct);
            return TypedResults.Ok(new WebsiteSummaryDto(await s.IlansizAracSayisiAsync(ct), rows.Count, rows.Count(r => r.Yayinda)));
        });

        g.MapGet("/havuz", async (WebIlanService s, CancellationToken ct) =>
            TypedResults.Ok<IReadOnlyList<VehicleClusterDto>>((await s.HavuzAsync(ct))
        ...
        g.MapGet("/ilanlar", async (int? sayfa, int? boyut, string? sirala, WebIlanService s, CancellationToken ct) =>
        ...
        g.MapGet("/ilanlar/{id:guid}", async Task<Results<Ok<ListingDetailDto>, ProblemHttpResult>> (
            Guid id, WebIlanService s, CancellationToken ct)
        ...
        g.MapPut("/ilanlar/{id:guid}/fiyat", async Task<Results<Ok<ListingPriceResultDto>, ProblemHttpResult>> (
            Guid id, ListingPriceRequest i, WebIlanService s, CancellationToken ct) =>
        {
            ...
            var copied = await s.AdimIkiAsync(id, i.GunlukFiyat.Value, i.HaftalikToplam, i.AylikToplam, i.KdvDahil, i.Surum!, ct);
        ...
        g.MapPut("/ilanlar/{id:guid}/ozellikler", async Task<Results<Ok<ListingFeaturesResultDto>, ProblemHttpResult>> (
            Guid id, ListingFeaturesRequest i, WebIlanService s, CancellationToken ct) =>
        {
            if (await s.GetAsync(id, ct) is null) return SystemApiCommon.NotFound("İlan bulunamadı.");
            SystemApiCommon.RequireVersion(i.Surum);
            var rows = (i.Satirlar ?? []).Select(r => new OzellikSatiri(r.Etiket ?? "", r.Deger ?? "", r.Gorunur)).ToList();
            var published = await s.AdimUcAsync(id, rows, i.Surum!, ct);
            return await ListingDetailAsync(id, s, ct) is { } d
        ...
        g.MapPost("/ilanlar/{id:guid}/durum", async Task<Results<Ok<ListingDetailDto>, ProblemHttpResult>> (
            Guid id, ListingStatusRequest i, WebIlanService s, CancellationToken ct) =>
        {
            if (await s.GetAsync(id, ct) is null) return SystemApiCommon.NotFound("İlan bulunamadı.");
            ...
            await s.SetDurumAsync(id, status, ct);
        ...
        g.MapDelete("/ilanlar/{id:guid}/fotograflar/{aracId:guid}/{fotoId:guid}",
            async Task<Results<Ok<IReadOnlyList<ListingPhotoDto>>, ProblemHttpResult>> (
                Guid id, Guid aracId, Guid fotoId, WebIlanService s, CancellationToken ct) =>
            {
                if (await s.GetAsync(id, ct) is null) return SystemApiCommon.NotFound("İlan bulunamadı.");
                // Fotoğraf BU ilanın üye aracına ait olmalı (başka aracın/ilanın foto kimliği 404).
                if ((await s.FotolarAsync(id, ct)).All(p => p.VehicleId != aracId || p.PhotoId != fotoId))
                    return SystemApiCommon.NotFound("Fotoğraf bulunamadı.");
                await s.FotoSilAsync(id, aracId, fotoId, ct);
    ...
    private static async Task<Results<Created<ListingCreatedDto>, ProblemHttpResult>> CreateListing(
        ListingCreateRequest i, WebIlanService s, CancellationToken ct)
    {
        ...
        {
            var id = separate
                ? await s.AdimBirAsync([.. (i.AracIdler ?? []).Where(g => g != Guid.Empty)], beraber: false, ct)
                : await s.AdimBirImzaAsync([.. (i.Imzalar ?? []).Where(x => !string.IsNullOrWhiteSpace(x))], ct);
    ...
    private static async Task<Results<Created<IReadOnlyList<ListingPhotoDto>>, ProblemHttpResult>> UploadListingPhoto(
        Guid id, IFormFile? foto, WebIlanService s, CancellationToken ct)
    {
        if (await s.GetAsync(id, ct) is null) return SystemApiCommon.NotFound("İlan bulunamadı.");
        ...
        await s.FotoEkleAsync(id, ms.ToArray(), ct); // içerikten tür tespiti + 2 MB + adet sınırı servistedir
    ...
    private static async Task<IReadOnlyList<ListingPhotoDto>> PhotosAsync(Guid id, WebIlanService s, CancellationToken ct)
        => (await s.FotolarAsync(id, ct)).Select(p => new ListingPhotoDto(p.VehicleId, p.PhotoId, p.Plaka, p.Sira)).ToList();

    /// <summary>Sürüm alanlardan ÖNCE okunur (arada yazım olursa istemci bayat sürümle 409 alır, sessiz ezme yok).</summary>
    private static async Task<ListingDetailDto?> ListingDetailAsync(Guid id, WebIlanService s, CancellationToken ct)
    {
        var version = await s.VersionAsync(id, ct);
        if (await s.GetAsync(id, ct) is not { } d) return null;
        var features = await s.OnerilenOzelliklerAsync(id, ct);
        return new ListingDetailDto(d.Ilan.Id, d.Ilan.Baslik, d.Ilan.Slug, d.Ilan.Durum.ToString(), d.Ilan.GunlukFiyat,
            d.Ilan.HaftalikToplam, d.Ilan.AylikToplam, d.Ilan.KdvDahil, await s.KardesTaslakSayisiAsync(id, ct),
    ...
    private static readonly SiralamaHaritasi<ListingRowDto> ListingSort = SiralamaHaritasi<ListingRowDto>
        .Olustur(x => x.Id).Alan("baslik", x => x.Baslik).Alan("durum", x => x.Durum).Alan("gunlukFiyat", x => x.GunlukFiyat)
