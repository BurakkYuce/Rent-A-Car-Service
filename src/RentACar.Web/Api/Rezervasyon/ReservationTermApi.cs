using Microsoft.AspNetCore.Http.HttpResults;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using RentACar.Application.Authorization;
using RentACar.Application.Bookings;
using RentACar.Application.Common;
using RentACar.Application.RezSartlar;
using RentACar.Domain.Common;
using RentACar.Domain.Entities;
using RentACar.Infrastructure.Persistence;
using RentACar.Web.Api.Kira;
using RentACar.Web.Identity;

namespace RentACar.Web.Api.Rezervasyon;

/// <summary>
/// <c>/api/ui/v1/rez-sartlari/*</c> — rez şartı (müşteri özel talebi) JSON uçları (F5.1). İş mantığı
/// <see cref="ReservationTermService"/>'te. İzin: OperationsWrite (menü kaydı + Blazor grubu). Para yok, defter yok.
/// <para><b>Üst kayıt kapsamı:</b> şart bir rezervasyona/teklife bağlıysa (<c>reservationId</c>/<c>quotationId</c>,
/// gevşek bağ — FK yok) o kaydın şube kapsamından geçer: kimlikli uçlarda ÖNCE bağlı kaydın kapsamı (başka şube →
/// 403), sonra işlem; listede kapsam dışı kayda bağlı şartlar görünmez; oluştur/güncellemede bağlanan kayıt kapsamda
/// ve bu kiracıda olmalı. Bağsız şart kiracı genelidir (Blazor ile aynı; müşterinin şube alanı yok).</para>
/// <para>PUT tam değiştirmedir: zorunlu <c>surum</c>, uyuşmazlık 409 <c>cakisma</c>.</para>
/// </summary>
public static class ReservationTermApi
{
    private const string Root = UiApiExtensions.V1 + "/rez-sartlari";

    public static RouteGroupBuilder MapReservationTermApi(this RouteGroupBuilder v1)
    {
        var g = v1.MapGroup("/rez-sartlari")
            .WithTags("Rez Şartı")
            .RequirePermission(Permission.OperationsWrite);

        g.MapGet("", GetList).MapFields(F5Shared.SortRules);
        g.MapGet("/gruplar", Groups);
        g.MapGet("/{id:guid}", Detail);
        g.MapPost("", Create).MapFields(WriteRules);
        g.MapPut("/{id:guid}", Update).MapFields(WriteRules);
        g.MapPost("/{id:guid}/karsilandi", Fulfilled);
        g.MapPost("/{id:guid}/geri-al", Undo);
        g.MapDelete("/{id:guid}", Delete);
        return g;
    }

    private static ProblemHttpResult NotFoundProblem() => F5Shared.NotFound("Rez şartı bulunamadı.");

    // ================================================================== kapsam

    /// <summary>
    /// Bağlı üst kaydın kapsamı. Rezervasyon/teklif servislerinin <c>GetAsync</c>'i kapsam dışında
    /// <see cref="NoPermissionException"/> atar (403). Bağlı kayıt artık yoksa (gevşek bağ) kapı açıktır — kayıt silinmez,
    /// yalnız iptal edilir; kaybolmuş bağ başka şubenin verisini sızdırmaz.
    /// </summary>
    private static async Task ParentRecordScopeAsync(
        RezSart s, ReservationService reservations, QuotationService quotations, CancellationToken ct)
    {
        if (s.ReservationId is Guid r) await reservations.GetAsync(r, ct);
        if (s.QuotationId is Guid t) await quotations.GetAsync(t, ct);
    }

    /// <summary>Kimlikli uçların ortak kapısı: yok/başka kiracı → null (404); bağlı kayıt kapsam dışı → 403.</summary>
    private static async Task<RezSart?> ComprehensiveAsync(
        Guid id, ReservationTermService terms, ReservationService reservations, QuotationService quotations, CancellationToken ct)
    {
        var s = await terms.GetAsync(id, ct);
        if (s is not null) await ParentRecordScopeAsync(s, reservations, quotations, ct);
        return s;
    }

    // ================================================================== okumalar

    private static readonly SortFieldMap<RezSartDto> Map = SortFieldMap<RezSartDto>
        .Create(s => s.Id)
        .Alan("talepTarihi", s => s.TalepTarihi)
        .Alan("musteri", s => s.MusteriAd)
        .Alan("grup", s => s.Grup)
        .Alan("sart", s => s.Sart)
        .Alan("karsilamaTarihi", s => s.KarsilamaTarihi);

    /// <summary>Blazor RezSartList süzgeçleri: müşteri, durum (<c>bekleyen</c>|<c>karsilanan</c>), talep günü aralığı.</summary>
    public sealed class ReservationTermListFilter
    {
        [FromQuery(Name = "musteriId")] public Guid? MusteriId { get; set; }
        /// <summary><c>bekleyen</c> (karşılanmadı) | <c>karsilanan</c> | boş (hepsi).</summary>
        [FromQuery(Name = "durum")] public string? Durum { get; set; }
        [FromQuery(Name = "bas")] public DateOnly? Bas { get; set; }
        [FromQuery(Name = "bit")] public DateOnly? Bit { get; set; }

        public RezSartFilter ToFilter()
        {
            var (min, max) = F5Shared.DayRange(Bas, Bit, "bas", "bit");
            return new RezSartFilter
            {
                MusteriId = MusteriId,
                Karsilandi = F5Shared.Nz(Durum)?.ToLowerInvariant() switch
                {
                    null => null,
                    "bekleyen" => false,
                    "karsilanan" => true,
                    _ => throw new ValidationException("Geçersiz durum değeri. İzin verilenler: bekleyen, karsilanan.", "durum"),
                },
                TarihBas = min,
                TarihBit = max,
            };
        }
    }

    private static async Task<Ok<Sayfa<RezSartDto>>> GetList(
        [AsParameters] ReservationTermListFilter f, ReservationTermService sartlar, ReservationService rezervasyonlar,
        QuotationService teklifler, ICurrentUser kullanici, IDbContextFactory<AppDbContext> dbf,
        int? sayfa, int? boyut, string? sirala, CancellationToken ct)
    {
        IEnumerable<RezSart> list = await sartlar.ListAsync(f.ToFilter(), ct);
        if (!BranchScope.EffectiveFilter(kullanici).Unrestricted)
        {
            // Kapsamlı kullanıcı: bağlı kaydı kapsamında OLMAYAN şartlar düşer (tekil uçların 403'üyle tutarlı).
            // Bağlı kaydın hiç olmadığı (kaybolmuş) bağ tekil kapıda açık → burada da görünür.
            var allReservations = list.Where(s => s.ReservationId is not null).Select(s => s.ReservationId!.Value).ToHashSet();
            var allSingle = list.Where(s => s.QuotationId is not null).Select(s => s.QuotationId!.Value).ToHashSet();
            var outerReservation = await OutOfScopeAsync(dbf, allReservations, (await rezervasyonlar.ListAsync(ct)).Select(r => r.Id), res: true, ct);
            var outerSingle = await OutOfScopeAsync(dbf, allSingle, (await teklifler.ListAsync(ct)).Select(t => t.Id), res: false, ct);
            list = list.Where(s => !(s.ReservationId is Guid r && outerReservation.Contains(r)) && !(s.QuotationId is Guid t && outerSingle.Contains(t)));
        }
        var rows = list.ToList();
        var customers = await F5Shared.CustomersAsync(dbf, rows.Select(s => s.MusteriId), ct);
        var dtos = rows.Select(s => RezSartDto.From(s, F5Shared.CustomerName(customers, s.MusteriId), null)).ToList();
        return TypedResults.Ok(F5Shared.Paginate(dtos, Map, sayfa, boyut, sirala));
    }

    /// <summary>Bağlı kimliklerden VAR olup kapsamda OLMAYANLAR (kapsamlı liste servisten; varlık RLS kapsamlı okumayla).</summary>
    private static async Task<HashSet<Guid>> OutOfScopeAsync(
        IDbContextFactory<AppDbContext> dbf, HashSet<Guid> linked, IEnumerable<Guid> comprehensive, bool res, CancellationToken ct)
    {
        if (linked.Count == 0) return [];
        var ids = linked.ToList();
        await using var db = await dbf.CreateDbContextAsync(ct);
        var existing = res
            ? await db.Reservations.AsNoTracking().Where(r => ids.Contains(r.Id)).Select(r => r.Id).ToListAsync(ct)
            : await db.Quotations.AsNoTracking().Where(q => ids.Contains(q.Id)).Select(q => q.Id).ToListAsync(ct);
        var visible = comprehensive.ToHashSet();
        return existing.Where(i => !visible.Contains(i)).ToHashSet();
    }

    /// <summary>Grup önerileri (serbest metin; mevcut şartlarda geçen gruplar — Blazor datalist'iyle aynı kaynak).</summary>
    private static async Task<Ok<IReadOnlyList<string>>> Groups(ReservationTermService sartlar, CancellationToken ct)
        => TypedResults.Ok<IReadOnlyList<string>>((await sartlar.ListAsync(null, ct))
            .Select(r => r.Grup).Where(g => !string.IsNullOrWhiteSpace(g)).Select(g => g!)
            .Distinct(StringComparer.Ordinal).OrderBy(g => g, StringComparer.Ordinal).ToList());

    private static async Task<Results<Ok<RezSartDto>, ProblemHttpResult>> Detail(
        Guid id, ReservationTermService sartlar, IReservationTermRepository depo, ReservationService rezervasyonlar, QuotationService teklifler,
        IDbContextFactory<AppDbContext> dbf, CancellationToken ct)
        => await DtoAsync(id, sartlar, depo, rezervasyonlar, teklifler, dbf, ct) is { } d ? TypedResults.Ok(d) : NotFoundProblem();

    /// <summary>Sürüm alanlardan ÖNCE okunur (bkz. kira detay).</summary>
    private static async Task<RezSartDto?> DtoAsync(
        Guid id, ReservationTermService terms, IReservationTermRepository store, ReservationService reservations, QuotationService quotations,
        IDbContextFactory<AppDbContext> dbf, CancellationToken ct)
    {
        var version = await store.VersionAsync(id, ct);
        var s = await ComprehensiveAsync(id, terms, reservations, quotations, ct);
        if (s is null) return null;
        var customers = await F5Shared.CustomersAsync(dbf, [s.MusteriId], ct);
        return RezSartDto.From(s, F5Shared.CustomerName(customers, s.MusteriId), version);
    }

    // ================================================================== yazmalar

    private static readonly (string, string)[] WriteRules =
    [
        ("Müşteri seçilmelidir", "musteriId"),
        ("Seçilen müşteri bulunamadı", "musteriId"),
        ("Şart/talep metni zorunludur", "sart"),
        ("Şart metni en çok", "sart"),
        ("Geçerlilik bitişi başlangıçtan önce", "bitTar"),
        ("Talep tarihi gelecekte", "talepTarihi"),
        ("Karşılama tarihi gelecekte", "karsilamaTarihi"),
    ];

    /// <summary>Uç sınırları (varchar) + bağlanan üst kaydın varlığı ve kapsamı.</summary>
    private static async Task<RezSartInput> InputAsync(
        RezSartIstegi i, ReservationService reservations, QuotationService quotations, CancellationToken ct)
    {
        RentalLimits.Text(i.Sart, 512, "sart", "Şart metni");
        RentalLimits.Text(i.Grup, 64, "grup", "Grup");
        RentalLimits.Text(i.TeslimEden, 128, "teslimEden", "Teslim eden");
        if (i.ReservationId is Guid r && await reservations.GetAsync(r, ct) is null) // kapsam dışı → 403
            throw new ValidationException("Bağlanan rezervasyon bulunamadı.", "reservationId");
        if (i.QuotationId is Guid t && await quotations.GetAsync(t, ct) is null)
            throw new ValidationException("Bağlanan teklif bulunamadı.", "quotationId");
        return new RezSartInput
        {
            MusteriId = i.MusteriId,
            Sart = i.Sart ?? "",
            Grup = F5Shared.Nz(i.Grup),
            BasTar = F5Shared.Utc(i.BasTar),
            BitTar = F5Shared.Utc(i.BitTar),
            TalepTarihi = F5Shared.Utc(i.TalepTarihi),
            KarsilamaTarihi = F5Shared.Utc(i.KarsilamaTarihi),
            TeslimEden = F5Shared.Nz(i.TeslimEden),
            ReservationId = i.ReservationId,
            QuotationId = i.QuotationId,
        };
    }

    private static async Task<Created<RezSartDto>> Create(
        RezSartIstegi istek, ReservationTermService sartlar, IReservationTermRepository depo, ReservationService rezervasyonlar,
        QuotationService teklifler, IDbContextFactory<AppDbContext> dbf, CancellationToken ct)
    {
        var id = await sartlar.CreateAsync(await InputAsync(istek, rezervasyonlar, teklifler, ct), ct);
        var dto = await DtoAsync(id, sartlar, depo, rezervasyonlar, teklifler, dbf, ct);
        return TypedResults.Created($"{Root}/{id}", dto!);
    }

    private static async Task<Results<Ok<RezSartDto>, ProblemHttpResult>> Update(
        Guid id, RezSartGuncelleIstegi istek, ReservationTermService sartlar, IReservationTermRepository depo,
        ReservationService rezervasyonlar, QuotationService teklifler, IDbContextFactory<AppDbContext> dbf, CancellationToken ct)
    {
        if (await ComprehensiveAsync(id, sartlar, rezervasyonlar, teklifler, ct) is null) return NotFoundProblem();
        if (string.IsNullOrWhiteSpace(istek.Surum))
            throw new ValidationException("Kayıt sürümü (surum) zorunludur; kaydı yeniden açın.", "surum");
        var input = await InputAsync(istek, rezervasyonlar, teklifler, ct);
        if (!await sartlar.UpdateAsync(id, input, istek.Surum, ct)) return NotFoundProblem();
        return await DtoAsync(id, sartlar, depo, rezervasyonlar, teklifler, dbf, ct) is { } d ? TypedResults.Ok(d) : NotFoundProblem();
    }

    /// <summary>Karşılandı işaretle. Zaten karşılanmışsa tarih DEĞİŞMEZ (servis kuralı) → çift tık zararsız.</summary>
    private static async Task<Results<Ok<RezSartDto>, ProblemHttpResult>> Fulfilled(
        Guid id, RezSartKarsilandiIstegi? istek, ReservationTermService sartlar, IReservationTermRepository depo,
        ReservationService rezervasyonlar, QuotationService teklifler, IDbContextFactory<AppDbContext> dbf, CancellationToken ct)
    {
        if (await ComprehensiveAsync(id, sartlar, rezervasyonlar, teklifler, ct) is null) return NotFoundProblem();
        RentalLimits.Text(istek?.TeslimEden, 128, "teslimEden", "Teslim eden");
        if (!await sartlar.MarkFulfilledAsync(id, F5Shared.Nz(istek?.TeslimEden), ct)) return NotFoundProblem();
        return await DtoAsync(id, sartlar, depo, rezervasyonlar, teklifler, dbf, ct) is { } d ? TypedResults.Ok(d) : NotFoundProblem();
    }

    private static async Task<Results<Ok<RezSartDto>, ProblemHttpResult>> Undo(
        Guid id, ReservationTermService sartlar, IReservationTermRepository depo, ReservationService rezervasyonlar,
        QuotationService teklifler, IDbContextFactory<AppDbContext> dbf, CancellationToken ct)
    {
        if (await ComprehensiveAsync(id, sartlar, rezervasyonlar, teklifler, ct) is null) return NotFoundProblem();
        if (!await sartlar.UndoFulfillmentAsync(id, ct)) return NotFoundProblem();
        return await DtoAsync(id, sartlar, depo, rezervasyonlar, teklifler, dbf, ct) is { } d ? TypedResults.Ok(d) : NotFoundProblem();
    }

    /// <summary>Silme (Blazor ile aynı izin: OperationsWrite — operasyonel not, mali belge değil).</summary>
    private static async Task<Results<NoContent, ProblemHttpResult>> Delete(
        Guid id, ReservationTermService sartlar, ReservationService rezervasyonlar, QuotationService teklifler, CancellationToken ct)
    {
        if (await ComprehensiveAsync(id, sartlar, rezervasyonlar, teklifler, ct) is null) return NotFoundProblem();
        return await sartlar.DeleteAsync(id, ct) ? TypedResults.NoContent() : NotFoundProblem();
    }
}

/// <summary>Rez şartı alanları (<c>POST /rez-sartlari</c>). PUT tam değiştirmedir (bağ kimlikleri dahil geri gönderilir).</summary>
public record RezSartIstegi
{
    public required Guid MusteriId { get; init; }
    public string? Sart { get; init; }
    public string? Grup { get; init; }
    public DateTimeOffset? BasTar { get; init; }
    public DateTimeOffset? BitTar { get; init; }
    /// <summary>Boş → oluştururken şimdi; güncellemede MEVCUT değer korunur.</summary>
    public DateTimeOffset? TalepTarihi { get; init; }
    public DateTimeOffset? KarsilamaTarihi { get; init; }
    public string? TeslimEden { get; init; }
    public Guid? ReservationId { get; init; }
    public Guid? QuotationId { get; init; }
}

/// <summary><c>PUT /rez-sartlari/{id}</c> — <c>surum</c> ZORUNLU.</summary>
public sealed record RezSartGuncelleIstegi : RezSartIstegi
{
    public string? Surum { get; init; }
}

public sealed record RezSartKarsilandiIstegi(string? TeslimEden);

/// <summary>Rez şartı. <c>Surum</c> yalnız tekil yanıtlarda dolu (listede null). <c>Karsilandi</c> = <c>KarsilamaTarihi</c> dolu.</summary>
public sealed record RezSartDto(
    Guid Id, Guid MusteriId, string MusteriAd, string Sart, string? Grup, DateTimeOffset? BasTar, DateTimeOffset? BitTar,
    DateTimeOffset TalepTarihi, DateTimeOffset? KarsilamaTarihi, bool Karsilandi, string? TeslimEden,
    Guid? ReservationId, Guid? QuotationId, string? Surum)
{
    public static RezSartDto From(RezSart s, string customerName, string? version) => new(
        s.Id, s.MusteriId, customerName, s.Sart, s.Grup, s.BasTar, s.BitTar, s.TalepTarihi, s.KarsilamaTarihi,
        s.KarsilamaTarihi is not null, s.TeslimEden, s.ReservationId, s.QuotationId, version);
}
