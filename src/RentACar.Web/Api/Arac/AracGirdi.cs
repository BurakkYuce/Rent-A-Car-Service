...
internal static class AracGirdi
{
    ...
    private static readonly (Func<AracIstegi, string?> Al, int En, string Alan, string Etiket)[] Metinler =
    [
    ...
    /// <summary>numeric(19,4) kolonları — uç sınırı <see cref="Sinirlar.EnFazlaTutar"/>.</summary>
    private static readonly (Func<AracIstegi, decimal?> Al, string Alan, string Etiket)[] Tutarlar =
    [
    ...
    private const int EnFazlaKm = 10_000_000;

    public static void Sinirla(AracIstegi i)
    {
        foreach (var (al, en, alan, etiket) in Metinler) Sinirlar.Metin(al(i), en, alan, etiket);
        foreach (var (al, alan, etiket) in Tutarlar) Sinirlar.Tutar(al(i), alan, etiket);
        foreach (var (deger, alan, etiket) in new (int?, string, string)[]
                 {
            ...
            if (deger is { } d && (d < 0 || d > EnFazlaKm))
                throw new ValidationException($"{etiket} 0 ile {EnFazlaKm:N0} arasında olmalıdır.", alan);
    }
    ...
    /// </summary>
    public static async Task HedefSubeKapsamiAsync(IBranchRepository subeler, ICurrentUser kullanici, string? sube, CancellationToken ct)
    {
        var s = F5Ortak.Nz(sube);
        if (s is null)
        {
    ...
    public static async Task CariVarligiAsync(IDbContextFactory<AppDbContext> dbf, Guid? cariId, CancellationToken ct)
    {
        if (cariId is not { } id) return;
    ...
    private static DateTimeOffset? U(DateTimeOffset? an) => F5Ortak.Utc(an);

    /// <summary>İstek → servis girdisi (enum adları doğrulanır, tarihler UTC).</summary>
    public static VehicleInput Girdi(AracIstegi i) => new()
    {
    ...
    public static AracKartDto Kart(Domain.Entities.Vehicle v, string? surum) => new()
    {
        Id = v.Id, Surum = surum, SubeId = v.SubeId, CreatedAtUtc = v.CreatedAtUtc, UpdatedAtUtc = v.UpdatedAtUtc,
