...
public static class CustomerView
{
    ...
    public const string AnonymousNameLabel = RentACar.Application.Customers.CustomerAnonymity.NameLabel;

    /// <summary><c>AnonimAd</c> işaretli (ya da bulunamayan) cariye hazır mesajdaki nötr hitap.</summary>
    public const string NeutralSalutation = "müşterimiz";
    ...
    public static string TarafAdi(Customer? m)
        => m is null ? "—" : m.AnonimAd ? AnonymousNameLabel : m.DisplayName;
    ...
    /// </summary>
    public static string ListName(string name, bool anonymousName) => RentACar.Application.Customers.CustomerAnonymity.Name(name, anonymousName);

    public static string? Phone(Customer? m) => m is null || m.AnonimTelefon ? null : m.CepTel;

    public static string? Email(Customer? m) => m is null || m.AnonimMail ? null : m.Email;
    ...
    public static string? Mask(string? v)
    {
        ...
        var open = v.Length >= 8 ? 4 : v.Length >= 5 ? 2 : 0;
        return new string('*', v.Length - open) + v[(v.Length - open)..];
    }
    ...
    /// </summary>
    public static string ShareMessage(RentalContract c, Customer? m)
    {
        static string Day(DateTimeOffset an) => TimeZoneInfo.ConvertTime(an, TenantDay.Slice).ToString("dd.MM.yyyy", Tr);
        var salutation = m is null || m.AnonimAd ? NeutralSalutation : m.DisplayName;
        return $"Sayın {salutation}, {c.SozlesmeNo} nolu kira sözleşmeniz: {Day(c.BasTar)} - {Day(c.BitTar)}, " +
    ...
    public static KiraMusteriOzeti Summary(Customer m)
    {
        var document = !m.AnonimBelge;
        var address = !m.AnonimAdres;
        return new KiraMusteriOzeti(
            ...
            Phone(m),
            Email(m),
            document ? Mask(m.EhliyetNo) : null,
            document ? Mask(m.PasaportNo) : null,
            document ? m.EhliyetSinifi : null,
            document ? m.EhliyetTarihi : null,
            document ? m.EhliyetYeri : null,
            document ? m.EhliyetUlke : null,
            document ? m.PasaportYeri : null,
