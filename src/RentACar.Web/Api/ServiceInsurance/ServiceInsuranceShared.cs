...
internal static class ServiceInsuranceShared
{
    ...
    public static ProblemHttpResult NotFound(string detail) => F5Ortak.Bulunamadi(detail);
    ...
    public static void PaymentAmount(decimal? value, string field)
    {
        ...
        if (v >= AracFinansOrtak.TutarUstSiniri) throw new ValidationException("Tutar çok büyük.", field);
        AracFinansOrtak.EnsureMaxScale(v, PaymentScale, field);
    }
    ...
    public static void RecordAmount(decimal? value, string field, bool allowNegative = false)
    {
        ...
        if (Math.Abs(v) >= AracFinansOrtak.TutarUstSiniri) throw new ValidationException("Tutar çok büyük.", field);
        AracFinansOrtak.EnsureMaxScale(v, PaymentScale, field);
    }
    ...
    public static void CatalogAmount(decimal? value, decimal? previous, string field, int scale = 4)
    {
        ...
        if (Math.Abs(v) >= AracFinansOrtak.TutarUstSiniri) throw new ValidationException("Tutar çok büyük.", field);
        AracFinansOrtak.EnsureMaxScale(v, scale, field);
    }
    ...
    public static void Ratio(decimal? value, decimal? previous, string field)
    {
        ...
        AracFinansOrtak.EnsureMaxScale(v, 4, field);
    }

    /// <summary>Text column length (trimmed, as the services store it).</summary>
    public static void Text(string? value, int max, string field) => AracFinansOrtak.Metin(value, max, field);
    ...
    public static DateTimeOffset? Date(DateTimeOffset? value, string field)
    {
        ...
        if (u.Year is < F5Ortak.MinQueryYear or > F5Ortak.MaxQueryYear)
            throw new ValidationException($"Tarih {F5Ortak.MinQueryYear} ile {F5Ortak.MaxQueryYear} yılları arasında olmalıdır.", field);
        return u;
    }
    ...
    public static Task<Dictionary<Guid, string>> PlatesAsync(IDbContextFactory<AppDbContext> dbf, IEnumerable<Guid> ids,
        CancellationToken ct) => F5Ortak.PlakalarAsync(dbf, ids, ct);
        ...
        IEnumerable<T> rows, Func<T, Guid?> vehicleOf, CancellationToken ct)
    {
        ...
        var branches = await AracFinansOrtak.AracSubeleriAsync(dbf,
            list.Select(vehicleOf).Where(v => v is not null).Select(v => v!.Value), ct);
        return list.Where(r => AracFinansOrtak.Gorunur(f, vehicleOf(r), branches)).ToList();
    }
    ...
    public static Task RecordScopeAsync(IDbContextFactory<AppDbContext> dbf, ICurrentUser user, Guid? vehicleId,
        CancellationToken ct) => AracFinansOrtak.KayitKapsamiAsync(dbf, user, vehicleId, ct);
    ...
    public static Task VehicleForWriteAsync(IDbContextFactory<AppDbContext> dbf, ICurrentUser user, Guid? vehicleId,
        string field, CancellationToken ct) => AracFinansOrtak.AracYazimAsync(dbf, user, vehicleId, field, zorunlu: true, ct);
    ...
    public static async Task<string> PlateAsync(IDbContextFactory<AppDbContext> dbf, Guid vehicleId, CancellationToken ct)
        => F5Ortak.Plaka(await F5Ortak.PlakalarAsync(dbf, [vehicleId], ct), vehicleId);

    /// <summary>Customer display name under the KVKK rule (<see cref="Kira.MusteriGorunumu"/>); null id → null.</summary>
    public static async Task<Dictionary<Guid, F5Ortak.CariGorunum>> CustomersAsync(IDbContextFactory<AppDbContext> dbf,
        IEnumerable<Guid?> ids, CancellationToken ct)
        => await F5Ortak.CarilerAsync(dbf, ids.Where(i => i is not null).Select(i => i!.Value), ct);

    public static string? CustomerName(Dictionary<Guid, F5Ortak.CariGorunum> map, Guid? id)
        => id is { } c ? F5Ortak.CariAdi(map, c) : null;
