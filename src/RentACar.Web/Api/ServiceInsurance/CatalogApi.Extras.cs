...
internal static partial class CatalogApi
{
    ...
    private static readonly CatalogSpec<ServisTanimService, ServisTanim, ServiceDefinitionRequest, ServiceDefinitionDto> ServiceDefinitions = new()
    {
        ...
        Sort = SiralamaHaritasi<ServiceDefinitionDto>.Olustur(x => x.Id).Alan("kod", x => x.Kod).Alan("aracTipi", x => x.AracTipi)
    ...
    private static void MapServiceDefinitions(RouteGroupBuilder v1)
    {
        ...
        g.MapGet("/oneriler", async Task<Ok<IReadOnlyList<ServiceDefinitionSuggestionDto>>> (ServisTanimService s, CancellationToken ct)
            => TypedResults.Ok<IReadOnlyList<ServiceDefinitionSuggestionDto>>((await s.OneriAsync(ct)).Select(o =>
        ...
        v1.MapGroup("").MapGet("/tarife-matris/fiyat", async Task<Results<Ok<RateMatrixPriceDto>, ProblemHttpResult>> (
                ...
                RateMatrixService s, CancellationToken ct) =>
            {
                ...
                var r = await s.CozumleAsync(new RateMatrisSorgu(F5Ortak.Nz(kanal), F5Ortak.Nz(sube), F5Ortak.Nz(aracGrupKod), an, g,
        ...
        v1.MapGroup("").MapGet("/kira-kurallari/ara", async Task<Ok<Sayfa<RentalRuleDto>>> (
                ...
                DateOnly? gecerliBit, int? sayfa, int? boyut, string? sirala, RentalRuleService s, CancellationToken ct) =>
            {
                ...
                {
                    Terim = F5Ortak.Nz(q), Durum = F5Ortak.EnumAdi<KampanyaDurum>(durum, "durum"),
                    TarihTipi = F5Ortak.EnumAdi<KuralTarihTipi>(tarihTipi, "tarihTipi"), KampanyaMi = kampanyaMi,
