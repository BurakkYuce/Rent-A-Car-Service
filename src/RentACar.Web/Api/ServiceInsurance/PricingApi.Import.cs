...
{
    private static readonly SiralamaHaritasi<RateMatrixDto> ImportSort = SiralamaHaritasi<RateMatrixDto>
        .Olustur(x => x.Id).Alan("kod", x => x.Kod).Alan("kanal", x => x.Kanal).Alan("sube", x => x.Sube)
    ...
    /// süzgecinden bağımsız, silmeyle AYNI saf yüklemle (<see cref="RateMatrixService.SilmeAdayi"/>) hesaplanır —
    ...
    {
        var state = F5Ortak.EnumAdi<TarifeOnayDurumu>(durum, "durum");
        ...
        int? toDelete = k is null ? null : all.Count(r => RateMatrixService.SilmeAdayi(r, k));
        return TypedResults.Ok(new RateImportView(
            ...
            rows.Count(r => r.OnayDurumu == TarifeOnayDurumu.Bekliyor), rows.Count(r => r.OnayDurumu == TarifeOnayDurumu.Onayli),
            toDelete));
    }
    ...
    private static async Task<Ok<RateChannelDeleteResult>> DeleteChannel(RateChannelDeleteRequest r, RateMatrixService svc,
        ...
        => TypedResults.Ok(new RateChannelDeleteResult(await svc.DeleteByKanalAsync(F5Ortak.Nz(r.Kanal), TarifeOnayDurumu.Bekliyor, ct)));
}
