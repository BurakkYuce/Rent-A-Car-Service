...
public static partial class DefinitionApi
{
    private static void MapRemainingDefinitions(RouteGroupBuilder v1)
    {
        ...
        v1.MapDefinition(new DefinitionRoute<HesapKodu, AccountCodeDto, AccountCodeRequest>
        {
            ...
            ValidateLimits = r =>
            {
                ...
                RentalLimits.Text(r.Ad, 200, "ad", "Ad");
    ...
    private static void ReservationSourceLimits(ReservationSourceRequest r)
    {
        ...
        RentalLimits.Text(r.Tedarikci, 128, "tedarikci", "Tedarikçi");
        RentalLimits.Text(r.SigortaKaynakNo, 64, "sigortaKaynakNo", "Sigorta kaynak no");
        RentalLimits.Text(r.DropKaynakNo, 64, "dropKaynakNo", "Drop kaynak no");
        RentalLimits.Text(r.ProvizyonSecenek, 64, "provizyonSecenek", "Provizyon seçeneği");
        RentalLimits.Text(r.MuafiyatSecenek, 64, "muafiyatSecenek", "Muafiyet seçeneği");
        RentalLimits.Text(r.MailAdres, 256, "mailAdres", "Mail adresi");
        RentalLimits.Amount(r.BebekKoltugu, "bebekKoltugu", "Bebek koltuğu");
        RentalLimits.Amount(r.Navigasyon, "navigasyon", "Navigasyon");
        RentalLimits.Amount(r.EkSurucu, "ekSurucu", "Ek sürücü");
        RentalLimits.Amount(r.Wifi, "wifi", "Wifi");
    }
    ...
    private static ReservationSourceInput ReservationSourceInputOf(ReservationSourceRequest b) => new()
    {
        ...
        KaynakGrubu = F5Shared.EnumAdi<ReservationSourceGroup>(b.KaynakGrubu, "kaynakGrubu"),
