...
public static partial class DefinitionApi
{
    private static void MapPricingDefinitions(RouteGroupBuilder v1)
    {
        v1.MapDefinition(new DefinitionRoute<DropTanim, DropDto, DropRequest>
        {
            ...
            ValidateLimits = r =>
            {
                ...
                RentalLimits.Text(r.Lokasyon, 150, "lokasyon", "Dönüş lokasyonu");
                RentalLimits.Text(r.Sube, 150, "sube", "Şube");
                RentalLimits.Text(r.CikisLokasyon, 150, "cikisLokasyon", "Çıkış lokasyonu");
                RentalLimits.Text(r.KarsilamaSekli, 100, "karsilamaSekli", "Karşılama şekli");
                RentalLimits.Text(r.CalismaSekli, 100, "calismaSekli", "Çalışma şekli");
                RentalLimits.Text(r.OzelIletisim, 200, "ozelIletisim", "Özel iletişim");
                RentalLimits.Amount(r.Ucret, "ucret", "Drop ücreti");
                RentalLimits.Amount(r.Drop2, "drop2", "Drop 2");
        ...
        }).MapFields([("Kod ön eki", "kodOnEk"), ("Ad ön eki", "adOnEk"), ("En az bir kademe", "kademeler"),
    ...
    private static void OccupancyLimits(string? code, string? name, string? group, string? branch)
    {
        RentalLimits.Text(code, 32, "kod", "Kod");
        RentalLimits.Text(name, 128, "ad", "Ad");
        RentalLimits.Text(group, 32, "aracGrupKod", "Araç grubu");
        RentalLimits.Text(branch, 64, "sube", "Şube");
    }
    ...
    private static DateTimeOffset? Start(DateOnly? day) => day is { } d ? F5Shared.DayStart(d) : null;
