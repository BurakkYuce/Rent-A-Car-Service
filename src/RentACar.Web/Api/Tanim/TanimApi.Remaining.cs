...
public static partial class TanimApi
{
    private static void MapRemainingDefinitions(RouteGroupBuilder v1)
    {
        ...
        v1.MapDefinition(new DefinitionRoute<HesapKodu, AccountCodeDto, AccountCodeRequest>
        {
            ...
            List = (sp, ct) => S<HesapKoduService>(sp).ListAsync(ct),
            Get = (sp, id, ct) => S<HesapKoduService>(sp).GetAsync(id, ct),
            GetVersion = (sp, id, ct) => S<HesapKoduService>(sp).GetVersionAsync(id, ct),
            GetVersions = (sp, ct) => S<HesapKoduService>(sp).GetVersionsAsync(ct),
            Create = (sp, b, ct) => S<HesapKoduService>(sp).CreateAsync(AccountCodeInputOf(b), ct),
            Update = (sp, id, b, v, ct) => S<HesapKoduService>(sp).UpdateAsync(id, AccountCodeInputOf(b), v, ct),
            Delete = (sp, id, ct) => S<HesapKoduService>(sp).DeleteAsync(id, ct),
            IdOf = e => e.Id,
    ...
    private static ReservationSourceInput ReservationSourceInputOf(ReservationSourceRequest b) => new()
    {
        ...
        KaynakGrubu = F5Ortak.EnumAdi<RezKaynakGrubu>(b.KaynakGrubu, "kaynakGrubu"),
