...
public static class DropDefinitionEndpoints
{
    public static IEndpointRouteBuilder MapDropDefinitionEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        grp.MapPost("/create", async (DropDefinitionService svc, HttpRequest req) =>
            await Run(req, () => svc.CreateAsync(Build(req.Form, active: true))));
        ...
        grp.MapPost("/update", async (DropDefinitionService svc, HttpRequest req, [FromForm] Guid id) =>
            await Run(req, () =>
            {
                ...
                return svc.UpdateAsync(id, Build(req.Form,
                    active: string.Equals(req.Form["aktif"].ToString(), "true", StringComparison.OrdinalIgnoreCase)));
            }));
    ...
    private static DropTanimInput Build(IFormCollection f, bool active) => new()
    {
    ...
    private static string Back(HttpRequest req, string? error = null)
    {
        ...
        foreach (var name in new[] { "fDonus", "fCikis", "fSube", "fAktif" })
        {
