...
public static class SiteContentEndpoints
{
    public static IEndpointRouteBuilder MapSiteContentEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        grp.MapPost("/sss/kaydet", async (SiteContentService svc, HttpRequest req) =>
        {
            ...
            catch (ValidationException ex) { return Back(ex, "&sekme=sss"); }
        });
    ...
    private static IResult Back(ValidationException ex, string extra = "")
        => Results.Redirect("/site-icerik?hata=" + Uri.EscapeDataString(ex.Message) + extra);
}
