...
/// </summary>
public sealed class ContractViewService(IConfiguration config, ILogger<ContractViewService> logger)
{
    public sealed record Sonuc(byte[] Pdf, string DosyaAdi);

    public async Task<Sonuc?> ViewAsync(string token, CancellationToken ct = default)
    {
        ...
        var name = new string(contractNo.Where(c => char.IsAsciiLetterOrDigit(c) || c is '-' or '_').ToArray());
        if (name.Length == 0) name = "sozlesme";
        return new Sonuc(pdf, $"Sozlesme-{name}.pdf");
    }
}
