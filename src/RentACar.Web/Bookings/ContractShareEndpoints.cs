...
/// </summary>
public static class ContractShareEndpoints
{
    public static IEndpointRouteBuilder MapContractShareEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            CancellationToken ct) =>
        {
            try
            {
                var existed = await paylasim.CancelAsync(id, ct);
                return After(id, existed ? "Paylaşım linki iptal edildi." : "Aktif paylaşım linki yok.");
            }
            catch (ValidationException ex) { return Error(id, ex); }
        });
    ...
    private static async Task<IResult> Isle(Guid id, ContractService contract, PdfExportService pdf,
        CancellationToken ct, Func<string, byte[], Task<PaylasimDurum>> operation)
    {
        try
        {
            ...
            await operation(s.SozlesmeNo, pdf.Contract(s));
            return After(id, "Paylaşım linki hazır.");
        }
        catch (ValidationException ex) { return Error(id, ex); }
    }

    // Statik SSR: POST sonrası kira ekranına dön (PRG deseni), sekme paylaşım barının olduğu yer.
    private static IResult After(Guid id, string message)
        => Results.Redirect($"/kiralar/{id}?paylasim={Uri.EscapeDataString(message)}");

    private static IResult Error(Guid id, ValidationException ex)
