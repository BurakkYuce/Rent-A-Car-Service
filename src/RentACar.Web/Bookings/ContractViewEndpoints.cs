...
/// </summary>
public static class ContractViewEndpoints
{
    public static IEndpointRouteBuilder MapContractViewEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapGet("/sozlesme/{token}", async (string token, ContractViewService svc,
            HttpResponse res, CancellationToken ct) =>
        {
            var result = await svc.ViewAsync(token, ct);
            if (result is null) return Results.NotFound();

            res.Headers.CacheControl = "no-store";
            res.Headers["X-Robots-Tag"] = "noindex, nofollow";
            res.Headers.ContentDisposition = $"inline; filename=\"{result.DosyaAdi}\"";
