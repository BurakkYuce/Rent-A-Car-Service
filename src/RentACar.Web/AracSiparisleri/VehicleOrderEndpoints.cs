...
public static class VehicleOrderEndpoints
{
    ...
    private static readonly string[] FilterFields = ["cariF", "araF", "aracF", "dosyaF", "durumF", "bas", "bit"];

    public static IEndpointRouteBuilder MapVehicleOrderEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        grp.MapPost("/create", async (VehicleOrderService svc, HttpRequest req) =>
            await Status(req, async () => await svc.CreateAsync(Build(req.Form)), "ok=1"));
    ...
    private static string Back(HttpRequest req, string? extra = null)
    {
        var q = new List<string>();
        foreach (var name in FilterFields)
        {
            var v = FormParse.Str(req.Form, name);
            if (v is not null) q.Add($"{name}={Uri.EscapeDataString(v)}");
        }
        if (extra is not null) q.Add(extra);
        return "/arac-siparis" + (q.Count > 0 ? "?" + string.Join("&", q) : "");
    }

    private static async Task<IResult> Status(HttpRequest req, Func<Task> action, string? successSuffix = null)
    {
        try { await action(); return Results.Redirect(Back(req, successSuffix)); }
        catch (ValidationException ex) { return Results.Redirect(Back(req, $"hata={Uri.EscapeDataString(ex.Message)}")); }
    }
}
