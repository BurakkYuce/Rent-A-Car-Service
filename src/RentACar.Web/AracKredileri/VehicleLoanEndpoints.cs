...
public static class VehicleLoanEndpoints
{
    ...
    private static readonly string[] FilterFields = ["cariF", "plakaF", "dosyaF", "durumF", "bas", "bit"];

    public static IEndpointRouteBuilder MapVehicleLoanEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        grp.MapPost("/create", async (VehicleLoanService svc, HttpRequest req) =>
        {
            ...
            return await Status(req, async () => { await svc.CreateAsync(input); }, "ok=1");
        });
        ...
        fin.MapPost("/taksit-ode", async (VehicleLoanService svc, HttpRequest req, [FromForm] Guid id,
            ...
            await Status(req, () => svc.PayInstallmentAsync(id,
        ...
        grp.MapPost("/iptal", async (VehicleLoanService svc, HttpRequest req, [FromForm] Guid id) =>
            await Status(req, () => svc.CancelAsync(id))).RequirePermission(Permission.OperationsDelete);
