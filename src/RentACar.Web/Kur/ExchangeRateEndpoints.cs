...
public static class ExchangeRateEndpoints
{
    public static IEndpointRouteBuilder MapExchangeRateEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        grp.MapPost("/yenile", async (TcmbExchangeRateService svc) =>
        {
        ...
        grp.MapPost("/sabit/kaydet", async (FixedExchangeRateService svc, HttpRequest req) =>
        {
            try
            {
                ...
                await svc.UpsertAsync(new SabitKurInput
                {
                    ...
                    BasTar = Day(f["basTar"].ToString()),
                    BitTar = Day(f["bitTar"].ToString()),
