...
/// </summary>
public static class TcmbExchangeRateParser
{
    public static IReadOnlyList<KurKaydi> Parse(string xml)
    {
        var root = XDocument.Parse(xml).Root; // <Tarih_Date Tarih="dd.mm.yyyy">
        var date = ParseDate(root?.Attribute("Tarih")?.Value);
        var list = new List<KurKaydi>();
        foreach (var c in root?.Elements("Currency") ?? [])
        {
            var code = (c.Attribute("Kod")?.Value ?? c.Attribute("CurrencyCode")?.Value ?? "").Trim().ToUpperInvariant();
            if (code.Length is < 2 or > 3) continue;
