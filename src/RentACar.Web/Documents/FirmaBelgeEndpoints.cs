...
/// </summary>
public static class FirmaBelgeEndpoints
{
    public static IEndpointRouteBuilder MapFirmaBelgeEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            HttpResponse res, CancellationToken ct) =>
        {
            ...
            var indirMi = req.Query.ContainsKey("indir");
            if (!indirMi) res.Headers.ContentDisposition = $"inline; filename=\"{icerik.DosyaAdi}\"";

            return Results.File(icerik.Bytes, "application/pdf",
                fileDownloadName: indirMi ? icerik.DosyaAdi : null,
