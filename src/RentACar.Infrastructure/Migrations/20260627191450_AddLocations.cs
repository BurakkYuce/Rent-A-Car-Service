    ...
    public partial class AddLocations : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateIndex(
                name: "IX_Locations_TenantId_Kod",
                table: "Locations",
                columns: new[] { "TenantId", "Kod" },
                unique: true);

            // RLS (tenant izolasyonu). Ofis master (mali belge DEĞİL) → değişmezlik trigger'ı
            // YOK; tam CRUD grant (yönetim ekranı düzenler/siler).
            migrationBuilder.Sql("ALTER TABLE \"Locations\" ENABLE ROW LEVEL SECURITY;");
            migrationBuilder.Sql("ALTER TABLE \"Locations\" FORCE ROW LEVEL SECURITY;");
            migrationBuilder.Sql("DROP POLICY IF EXISTS tenant_isolation ON \"Locations\";");
            migrationBuilder.Sql(
                "CREATE POLICY tenant_isolation ON \"Locations\" " +
                "USING (\"TenantId\" = NULLIF(current_setting('app.tenant_id', true), '')::uuid) " +
                "WITH CHECK (\"TenantId\" = NULLIF(current_setting('app.tenant_id', true), '')::uuid);");
            migrationBuilder.Sql("GRANT SELECT, INSERT, UPDATE, DELETE ON \"Locations\" TO racar_app;");
        }
