    ...
    public partial class AddBranches : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateIndex(
                name: "IX_Branches_TenantId_Kod",
                table: "Branches",
                columns: new[] { "TenantId", "Kod" },
                unique: true);

            // RLS (tenant izolasyonu). Şube master kaydı (mali belge DEĞİL) → değişmezlik
            // trigger'ı YOK; tam CRUD grant (yönetim ekranı düzenler/siler).
            migrationBuilder.Sql("ALTER TABLE \"Branches\" ENABLE ROW LEVEL SECURITY;");
            migrationBuilder.Sql("ALTER TABLE \"Branches\" FORCE ROW LEVEL SECURITY;");
            migrationBuilder.Sql("DROP POLICY IF EXISTS tenant_isolation ON \"Branches\";");
            migrationBuilder.Sql(
                "CREATE POLICY tenant_isolation ON \"Branches\" " +
                "USING (\"TenantId\" = NULLIF(current_setting('app.tenant_id', true), '')::uuid) " +
                "WITH CHECK (\"TenantId\" = NULLIF(current_setting('app.tenant_id', true), '')::uuid);");
            migrationBuilder.Sql("GRANT SELECT, INSERT, UPDATE, DELETE ON \"Branches\" TO racar_app;");
        }
