...
public static class SharedQueries
{
    ...
    public static async Task<(decimal FaturalananBrut, int FarkSayisi)> DifferenceStateAsync(
        AppDbContext db, Guid rentalId, CancellationToken ct = default)
    {
        var rentalInvoices = await db.Invoices.AsNoTracking()
            ...
            .ToListAsync(ct);
        var gross = rentalInvoices.Sum(x => x.GenelToplam);
        var refundGross = 0m;
        if (gross != 0m)
        {
            var ids = rentalInvoices.Select(x => x.Id).ToList();
            refundGross = await db.Invoices.AsNoTracking()
        ...
        var differenceCount = await db.Invoices.AsNoTracking()
            .CountAsync(i => i.KaynakKiraId == rentalId && i.Durum != Domain.Enums.InvoiceStatus.Iptal, ct);
        return (gross - refundGross, differenceCount);
    }
    ...
    public static async Task<IReadOnlyList<VadeSource>> DueSourcesAsync(AppDbContext db, CancellationToken ct = default)
    {
    ...
    public static async Task<(int Adet, decimal TutarTl)> CollectionTryAsync(
    ...
    public static Task<List<VehicleStatus>> VehicleStatusesAsync(AppDbContext db, CancellationToken ct = default)
    ...
    public static async Task<TutSatHamPaket> HoldSellRawAsync(
        AppDbContext db, DateTimeOffset now, CancellationToken ct = default)
    {
        var last12Start = now.AddMonths(-12);
        var previous12Start = now.AddMonths(-24);
        ...
        var kmByVeh = (await db.Rentals.AsNoTracking()
            ...
            .ToDictionary(g => g.Key, g => (
                Km12: g.Where(k => k.Bit >= last12Start).Sum(k => k.DonusKm!.Value - k.CikisKm!.Value),
                KmOnceki: g.Where(k => k.Bit >= previous12Start && k.Bit < last12Start).Sum(k => k.DonusKm!.Value - k.CikisKm!.Value)));

        var raw = vehicles.Select(a =>
        {
            var g = expense.GetValueOrDefault(a.Id);
            var km = kmByVeh.GetValueOrDefault(a.Id);
            return new FiloTutSatRow(a.Id, g.G12, g.GOnceki, km.Km12, km.KmOnceki);
        }).ToList();
        return new TutSatHamPaket(raw, vehicles);
    }
    ...
    /// </param>
    public static async Task<IReadOnlyList<PeriyodikServisRow>> PeriodicServiceAsync(
        AppDbContext db, CancellationToken ct = default,
        RentACar.Application.Reporting.PeriyodikServisFilter? filter = null)
    {
        var maintenance = (await db.ServiceRecords.AsNoTracking()
        ...
        var definitions = (await db.ServisTanimlari.AsNoTracking()
        ...
        var lastService = (await db.ServiceRecords.AsNoTracking()
        ...
        return vehicles.Select(v =>
            {
                int? serviceTarget = maintenance.TryGetValue(v.Id, out var s) ? s : null;
                int? autoTarget = v.SonBakimKm is int last && v.Tip is { } tip
                    && definitions.TryGetValue(tip.Trim(), out var range) ? last + range : null;

                var (target, source) = (servisHedef: serviceTarget, otoHedef: autoTarget) switch
                {
                ...
                var ss = lastService.GetValueOrDefault(v.Id);
                return new PeriyodikServisRow(v.Id, v.Plaka, v.Km, target, target - v.Km, source,
                    v.Marka, v.Tip, v.ModelYili, v.Yakit?.ToString(), v.Vites?.ToString(), v.Sube,
                    ss?.GirisTarihi, ss?.GirisKm, v.Aktif);
            })
            .Where(r => IsEligible(r, filter))
            .OrderBy(r => r.KalanKm ?? int.MaxValue)
            .ToList();
    }

    /// <summary>FAZ-76 filtre uygunluğu. Filtre null ise HER satır geçer (eski davranış).</summary>
    private static bool IsEligible(PeriyodikServisRow r, RentACar.Application.Reporting.PeriyodikServisFilter? f)
    {
        ...
        if (f.UyariEsigi is int threshold && r.KalanKm is int remaining && remaining > threshold) return false;
        return true;
    }
}
