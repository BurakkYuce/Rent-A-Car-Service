...
/// </summary>
public sealed class AddOnDefinitionRepository(IDbContextFactory<AppDbContext> factory) : IAddOnDefinitionRepository
{
    ...
    public async Task CreateAsync(EkHizmetTanim definition, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        db.EkHizmetTanimlari.Add(definition);
        try
        {
        ...
        {
            throw new ValidationException($"'{definition.Kod}' kodlu ek hizmet zaten var.");
        }
    }
