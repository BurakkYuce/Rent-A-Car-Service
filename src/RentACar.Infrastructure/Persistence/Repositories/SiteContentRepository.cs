...
/// </summary>
public sealed class SiteContentRepository(IDbContextFactory<AppDbContext> factory) : ISiteContentRepository
{
    ...
    public async Task<bool> SlugExistsAsync(string slug, Guid? excludedId, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await db.SayfaIcerikler.AsNoTracking()
            .AnyAsync(s => s.Slug == slug && (excludedId == null || s.Id != excludedId), ct);
    }

    public async Task AddAsync(SayfaIcerik page, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        db.SayfaIcerikler.Add(page);
    ...
    public async Task<IReadOnlyList<SssSatiri>> ListFaqAsync(bool publishedOnly, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        ...
        return await db.SssKayitlari.AsNoTracking()
            .Where(k => !publishedOnly || k.Yayinda)
            .OrderBy(k => k.Sira).ThenBy(k => k.Soru)
            .Select(k => new SssSatiri(k.Id, k.Soru, k.Cevap, k.Sira, k.Yayinda))
            .ToListAsync(ct);
    }

    public async Task AddFaqAsync(SssKaydi record, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        db.SssKayitlari.Add(record);
        await db.SaveChangesAsync(ct);
    }
    ...
    public Task<bool> UpdateAsync(Guid id, string? expectedVersion, Action<SayfaIcerik> apply, CancellationToken ct = default)
        => RowVersionSql.UpdateAsync(_factory, RowVersionSql.PageContents, id, expectedVersion,
    ...
    public Task<bool> UpdateFaqAsync(Guid id, string? expectedVersion, Action<SssKaydi> apply, CancellationToken ct = default)
        => RowVersionSql.UpdateAsync(_factory, RowVersionSql.FaqEntries, id, expectedVersion,
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.PageContents, id, ct);
    }

    public async Task<string?> FaqVersionAsync(Guid id, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.FaqEntries, id, ct);
    }
}
