...
/// </summary>
public sealed class MessageRepository(IDbContextFactory<AppDbContext> factory) : IMessageRepository, IMessageTemplateVersionStore
{
    private readonly IDbContextFactory<AppDbContext> _factory = factory;

    /// <summary>F11.1b — şablon sürümleri (xmin). Kimlikler önce query filter'lı EF sorgusuyla alınır (kiracı kapsamı
    /// RLS'e ek olarak uygulamada da); sürüm her satır için <see cref="RowVersionSql.ReadAsync"/> ile okunur.</summary>
    public async Task<IReadOnlyDictionary<Guid, string>> VersionsAsync(CancellationToken ct = default)
    {
        ...
        foreach (var id in ids)
            if (await RowVersionSql.ReadAsync(db, RowVersionSql.MessageTemplates, id, ct) is { } v)
                result[id] = v;
        return result;
    }
    ...
    public async Task UpsertAsync(MesajSablonInput input, string? expectedVersion, CancellationToken ct = default)
    {
        try
        {
            await PgRetry.RunAsync(async () =>
            {
                ...
                if (id is { } k)
                {
                    await RowVersionSql.LockAsync(db, RowVersionSql.MessageTemplates, k, ct);
                    var current = await RowVersionSql.ReadAsync(db, RowVersionSql.MessageTemplates, k, ct);
    ...
    public async Task<MesajSablon?> FindTemplateAsync(MessageType type, MessageChannel channel, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await db.MesajSablonlari.AsNoTracking()
            .FirstOrDefaultAsync(x => x.Tur == type && x.Kanal == channel, ct);
    }
    ...
    public async Task UpsertTemplateAsync(MesajSablonInput input, CancellationToken ct = default)
    {
        ...
        s.Aktif = input.Aktif;
        s.UpdatedAtUtc = DateTimeOffset.UtcNow;
        if (newItem) db.MesajSablonlari.Add(s);
        ...
        catch (DbUpdateException ex) when (ex.InnerException is PostgresException { SqlState: PostgresErrorCodes.UniqueViolation })
        {
            ...
            var existing = await db.MesajSablonlari
                .FirstAsync(x => x.Tur == input.Tur && x.Kanal == input.Kanal, ct);
            existing.Konu = string.IsNullOrWhiteSpace(input.Konu) ? null : input.Konu.Trim();
            existing.Govde = input.Govde.Trim();
    ...
    public async Task<GidenMesaj?> FindMessageAsync(string key, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await db.GidenMesajlar.AsNoTracking().FirstOrDefaultAsync(x => x.Anahtar == key, ct);
    }
    ...
    /// </summary>
    public static Task<List<GidenMesaj>> QueuedAsync(
        AppDbContext db, DateTimeOffset now, int maxAttempts, CancellationToken ct = default)
    {
        ...
        return db.GidenMesajlar
            .Where(x => x.Durum == OutgoingMessageStatus.Kuyrukta
                        && x.DenemeSayisi < maxAttempts
                        && x.OlusturmaUtc >= limit)
