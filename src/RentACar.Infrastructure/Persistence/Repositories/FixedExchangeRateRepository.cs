...
/// </summary>
public sealed class FixedExchangeRateRepository(IDbContextFactory<AppDbContext> factory) : IPinnedRateRepository
{
    ...
    public async Task<SabitKur?> GetActiveAsync(string code, DateTimeOffset date, CancellationToken ct = default)
    {
        var k = code.Trim().ToUpperInvariant();
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await db.SabitKurlar.AsNoTracking()
            .Where(x => x.Kod == k && x.Aktif
                && (x.BasTar == null || x.BasTar <= date)
                && (x.BitTar == null || date <= x.BitTar))
    ...
    public async Task CreateAsync(SabitKur fixedValue, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        db.SabitKurlar.Add(fixedValue);
        try { await db.SaveChangesAsync(ct); }
        catch (DbUpdateException ex) when (ex.InnerException is PostgresException { SqlState: PostgresErrorCodes.UniqueViolation })
        { throw new ValidationException($"'{fixedValue.Kod}' için sabit kur zaten var."); }
    }

    public async Task<bool> UpdateAsync(SabitKur fixedValue, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        var existing = await db.SabitKurlar.FirstOrDefaultAsync(x => x.Id == fixedValue.Id, ct);
        if (existing is null) return false;
        existing.Kod = fixedValue.Kod;
        existing.Kur = fixedValue.Kur;
        existing.BasTar = fixedValue.BasTar;
        existing.BitTar = fixedValue.BitTar;
        existing.Aktif = fixedValue.Aktif;
