...
/// </summary>
public abstract class MasterDefinitionRepository<T>(IDbContextFactory<AppDbContext> factory, string singularName)
    : IMasterDefinitionRepository<T> where T : class, IMasterDefinition
{
    ...
    public async Task CreateAsync(T entity, CancellationToken ct = default)
    {
        ...
        {
            throw new ValidationException($"'{entity.Kod}' kodlu {_singularName} zaten var.");
        }
    }
    ...
    public async Task<bool> DeleteAsync(Guid id, CancellationToken ct = default)
    {
        ...
        catch (DbUpdateException ex) when (ex.InnerException is PostgresException { SqlState: PostgresErrorCodes.ForeignKeyViolation })
        {
            ...
            throw new ValidationException(DefinitionMessages.InUse(_singularName));
        }
        return true;
    }
    ...
    public Task<bool> UpdateAsync(Guid id, string expectedVersion, Action<T> apply, CancellationToken ct = default)
        => RowVersion.UpdateAsync(_factory, id, expectedVersion, apply, e => $"'{e.Kod}' kodlu {_singularName} zaten var.", ct);
}
