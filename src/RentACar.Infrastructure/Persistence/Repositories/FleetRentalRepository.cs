...
public sealed class FleetRentalRepository(IDbContextFactory<AppDbContext> factory) : IFleetRentalRepository
{
    ...
    public Task<bool> UpdateAsync(Guid id, string? expectedVersion, Action<FiloKiralama> apply, CancellationToken ct = default)
        => RowVersionSql.UpdateAsync(_factory, RowVersionSql.FleetRentals, id, expectedVersion,
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.FleetRentals, id, ct);
    }
    ...
    public async Task CreateAsync(FiloKiralama row, CancellationToken ct = default)
    {
        await PgRetry.RunAsync(async () => // P0-5: deadlock/serialization çakışmasında baştan dene
        {
            ...
            row.No = await DocumentNoGenerator.GenerateAsync(db, db.TenantId, DocumentNoType.FiloKiralama, ct);
    ...
    public async Task<bool> SetStatusAsync(Guid id, FleetRentalStatus status, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        var row = await db.FiloKiralamalar.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (row is null) return false;
