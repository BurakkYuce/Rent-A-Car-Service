...
/// </summary>
public sealed class PlatformDocumentRepository(IDbContextFactory<AppDbContext> factory) : IPlatformDocumentRepository
{
    private readonly IDbContextFactory<AppDbContext> _factory = factory;

    /// <summary>Son bu kadar günde güncellenen belge "Yeni" etiketi alır.</summary>
    private const int NewDayCount = 14;
    ...
    /// </summary>
    private static IQueryable<PlatformBelge> Visible(AppDbContext db, Guid tenantId, bool isManager)
        => db.PlatformBelgeler.AsNoTracking()
            ...
            .Where(b => !b.YalnizYoneticiler || isManager);

    public async Task<IReadOnlyList<FirmaBelgeSatiri>> ListAsync(
        Guid tenantId, bool isManager, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        var threshold = DateTimeOffset.UtcNow.AddDays(-NewDayCount);
        // Select PROJEKSİYONU: `Bytes` kolonuna DOKUNULMAZ (bytea/TOAST faturası).
        return await Visible(db, tenantId, isManager)
            .OrderByDescending(b => b.GuncellemeUtc).ThenBy(b => b.Baslik)
            .Select(b => new FirmaBelgeSatiri(
                b.Id, b.Baslik, b.Aciklama, b.Surum, b.GuncellemeUtc, b.Boyut, b.GuncellemeUtc >= threshold))
            .ToListAsync(ct);
    }

    public async Task<BelgeIcerik?> DownloadAsync(
        Guid documentId, Guid tenantId, bool isManager, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
