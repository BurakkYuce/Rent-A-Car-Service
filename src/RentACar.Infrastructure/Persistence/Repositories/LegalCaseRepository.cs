...
/// </summary>
public sealed class LegalCaseRepository(IDbContextFactory<AppDbContext> factory) : ILegalCaseRepository
{
        ...
        HukukDosyaFilter? filter = null, CancellationToken ct = default)
    {
        ...
        if (filter is not null)
        {
            ...
            if (filter.Tur is { } type) q = q.Where(x => x.h.Tur == type);
            if (filter.Durum is { } status) q = q.Where(x => x.h.Durum == status);
            if (filter.Bas is { } start) q = q.Where(x => x.h.Tarih >= start);
    ...
    public async Task<bool> FileNoExistsAsync(string fileNo, Guid? excludeId = null, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        var k = fileNo.Trim().ToUpperInvariant();
    ...
    /// <summary>F7.1 — satır kilidi + iyimser sürüm (<see cref="RowVersionSql"/>); DosyaNo yarışı yine 400.</summary>
    public async Task<bool> UpdateAsync(Guid id, string expectedVersion, Action<HukukDosya> apply, CancellationToken ct = default)
    {
        string? fileNo = null;
        try
        {
            return await RowVersionSql.UpdateAsync(_factory, RowVersionSql.LegalFiles, id, expectedVersion,
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.LegalFiles, id, ct);
    }
