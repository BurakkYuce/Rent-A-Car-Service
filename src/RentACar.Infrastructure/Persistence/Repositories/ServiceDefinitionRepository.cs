...
public sealed class ServiceDefinitionRepository(IDbContextFactory<AppDbContext> factory) : IServiceDefinitionRepository
{
    ...
    public async Task<IReadOnlyList<FiloKombinasyon>> FleetCombinationsAsync(CancellationToken ct = default)
    {
        ...
        return raw
            ...
            .Select(g => new FiloKombinasyon(
                Representative(g.Select(x => x.Marka)),
                Representative(g.Select(x => x.Tip)),
                Representative(g.Select(x => x.Yakit?.ToString())),
                Representative(g.Select(x => x.Vites?.ToString())),
                g.Count()))
    ...
    /// </summary>
    private static string? Representative(IEnumerable<string?> values)
        => values.Select(x => x?.Trim())
