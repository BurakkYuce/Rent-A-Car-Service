...
public sealed class VehicleLoanRepository(IDbContextFactory<AppDbContext> factory) : IVehicleLoanRepository
{
    ...
    public async Task<IReadOnlyList<AracKredi>> SearchAsync(AracKrediFilter filter, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        ...
        if (filter.Bas is { } start) q = q.Where(x => x.BaslangicTarihi >= start);
        if (filter.Bit is { } bit) q = q.Where(x => x.BaslangicTarihi <= bit);
    ...
    public async Task CreateAsync(AracKredi row, CancellationToken ct = default)
    {
        await PgRetry.RunAsync(async () => // P0-5: deadlock/serialization çakışmasında baştan dene
        {
            ...
            row.No = await DocumentNoGenerator.GenerateAsync(db, db.TenantId, DocumentNoType.AracKredi, ct);
            ...
            catch (Microsoft.EntityFrameworkCore.DbUpdateException ex) when (PkViolation.Is(ex))
            {
                await tx.RollbackAsync(ct);
    ...
    private const string InstallmentDuplicate = "Bu taksit ödemesi zaten kaydedilmiş (çift gönderim).";
        ...
        CancellationToken ct = default, Guid? operationKey = null, int? expectedSequence = null)
    {
        return await PgRetry.RunAsync(async () => // P0-5 deadlock retry + sayaç yarışı koruması
        {
            ...
            if (operationKey is Guid key && key != Guid.Empty &&
                await db.Expenses.AsNoTracking().AnyAsync(e => e.IslemAnahtari == key, ct))
                throw new RentACar.Application.Common.DuplicateOperationException(InstallmentDuplicate);
            ...
            if (expectedSequence is int expected && row.OdenenTaksit + 1 != expected)
                ...
                when (ex.InnerException is Npgsql.PostgresException { SqlState: Npgsql.PostgresErrorCodes.UniqueViolation })
            {
                ...
                await tx.RollbackAsync(ct);
                throw IdempotencyConstraint.Red(ex, InstallmentDuplicate);
            }
            return true;
        }, ct);
    }
    ...
    public async Task<bool> SetStatusAsync(Guid id, LoanStatus status, CancellationToken ct = default)
    {
        return await PgRetry.RunAsync(async () =>
        {
            await using var db = await _factory.CreateDbContextAsync(ct);
