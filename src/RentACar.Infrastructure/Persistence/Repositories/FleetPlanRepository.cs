...
public sealed class FleetPlanRepository(IDbContextFactory<AppDbContext> factory) : IFleetPlanRepository
{
    ...
    public async Task<bool> UpdateLockedAsync(Guid id, string? expectedVersion, Action<FiloPlanHedefi> apply,
        CancellationToken ct = default)
    {
        try
        {
            return await RowVersionSql.UpdateAsync(_factory, RowVersionSql.FleetPlanTargets, id, expectedVersion,
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.FleetPlanTargets, id, ct);
    }
