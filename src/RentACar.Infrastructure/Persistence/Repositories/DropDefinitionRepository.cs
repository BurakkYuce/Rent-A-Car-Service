...
public sealed class DropDefinitionRepository(IDbContextFactory<AppDbContext> factory) : IDropDefinitionRepository
{
    ...
    public async Task<IReadOnlyList<DropTanim>> SearchAsync(
        RentACar.Application.DropTanimlari.DropTanimFilter filter, CancellationToken ct = default)
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        ...
        if (filter.Aktif is bool a) q = q.Where(x => x.Aktif == a);
        var rows = await q.ToListAsync(ct);

        static bool Es(string? x, string? v)
            => string.Equals((x ?? "").Trim(), (v ?? "").Trim(), StringComparison.OrdinalIgnoreCase);

        IEnumerable<DropTanim> result = rows;
        if (!string.IsNullOrWhiteSpace(filter.DonusLokasyon))
            result = result.Where(x => Es(x.Lokasyon, filter.DonusLokasyon));
        if (!string.IsNullOrWhiteSpace(filter.CikisLokasyon))
            result = result.Where(x => x.CikisLokasyon is not null && Es(x.CikisLokasyon, filter.CikisLokasyon));
        if (!string.IsNullOrWhiteSpace(filter.Sube))
            result = result.Where(x => Es(x.Sube, filter.Sube));
