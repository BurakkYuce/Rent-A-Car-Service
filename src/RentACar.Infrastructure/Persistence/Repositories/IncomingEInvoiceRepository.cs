...
/// </summary>
public sealed class IncomingEInvoiceRepository(IDbContextFactory<AppDbContext> factory) : IIncomingEInvoiceRepository
{
        ...
        GelenEFaturaFilter? filter, CancellationToken ct = default)
    {
        ...
        if (filter is not null)
        {
            ...
            if (filter.Bas is { } start) q = q.Where(r => r.Tarih >= start);
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.IncomingEInvoices, id, ct);
    }

    public Task<bool> UpdateLockedAsync(Guid id, string? expectedVersion, Action<GelenEFatura> apply, CancellationToken ct = default)
        => RowVersionSql.UpdateAsync(_factory, RowVersionSql.IncomingEInvoices, id, expectedVersion,
