...
/// </summary>
public sealed class VatRateRepository(IDbContextFactory<AppDbContext> factory) : IVatRateRepository
{
    ...
    public async Task<bool> UpdateAsync(Guid id, string? expectedVersion, Action<KdvRate> apply, CancellationToken ct = default)
    {
        ...
        try
        {
            return await RowVersionSql.UpdateAsync(_factory, RowVersionSql.KdvRates, id, expectedVersion,
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.KdvRates, id, ct);
    }
