...
/// </summary>
public sealed class PersonnelRepository(IDbContextFactory<AppDbContext> factory) : IPersonnelRepository
{
    ...
    public async Task<bool> UpdateAsync(Guid id, string? expectedVersion, Action<Personel> apply, CancellationToken ct = default)
    {
        ...
        try
        {
            return await RowVersionSql.UpdateAsync(_factory, RowVersionSql.Personnel, id, expectedVersion,
    ...
    {
        await using var db = await _factory.CreateDbContextAsync(ct);
        return await RowVersionSql.ReadAsync(db, RowVersionSql.Personnel, id, ct);
    }
