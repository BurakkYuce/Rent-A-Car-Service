...
/// <list type="bullet">
/// <item><see cref="InvoiceAsync"/>: kira-fatura advisory kilidi (<c>fatura:{tenant}:{kira}</c>; adversarial
/// B2-Kritik-1). Base/fark/dönem faturası kesimi ve kira İPTALİ bununla serileşir.</item>
/// <item><see cref="RowAsync"/>: kira satırı <c>FOR UPDATE</c>. Kira toplamını/durumunu değiştiren her yol
...
/// </summary>
internal static class RentalLocks
{
    public static async Task InvoiceAsync(AppDbContext db, Guid rentalId, CancellationToken ct)
    {
    ...
    public static Task RowAsync(AppDbContext db, Guid rentalId, CancellationToken ct)
    ...
    /// </summary>
    public static async Task<string?> VersionAsync(AppDbContext db, Guid rentalId, CancellationToken ct)
    {
        var conn = db.Database.GetDbConnection();
    ...
    public static async Task CancelledRentalHasNoInvoiceAsync(AppDbContext db, Guid rentalId, CancellationToken ct)
    {
    ...
    /// </summary>
    public static async Task IsBaseInvoiceCurrentAsync(AppDbContext db, Guid rentalId, decimal invoiceGross, CancellationToken ct)
    {
        var rental = await db.Rentals.AsNoTracking().FirstOrDefaultAsync(r => r.Id == rentalId, ct);
        if (rental is null) return;
        var attachments = await db.RentalAddOns.AsNoTracking().Where(a => a.RentalId == rentalId)
            .Select(a => a.NetTutar + a.KdvTutar).ToListAsync(ct);
        var current = RentACar.Application.Finance.VatMath.RoundGross(RentACar.Application.Bookings.RentalTotals.BaseGross(rental))
                     + attachments.Sum();
        if (current != invoiceGross)
    ...
    public static Task<bool> HasOpenInvoiceAsync(AppDbContext db, Guid rentalId, CancellationToken ct)
