...
/// </summary>
internal static class DefterKumesi
{
    /// <summary>İki küme hedef + tutar bakımından birebir aynı mı (sıra önemsiz, çoklu küme).</summary>
    public static bool Ayni(IEnumerable<AccountLedgerEntry> mevcut, IEnumerable<AccountLedgerEntry> gelen)
    {
        var a = mevcut.Select(Imza).OrderBy(x => x, StringComparer.Ordinal).ToList();
        var b = gelen.Select(Imza).OrderBy(x => x, StringComparer.Ordinal).ToList();
    ...
    public static Task<List<AccountLedgerEntry>> OkuAsync(
    ...
    private static string Imza(AccountLedgerEntry e) => string.Create(CultureInfo.InvariantCulture,
