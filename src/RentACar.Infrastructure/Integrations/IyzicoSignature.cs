...
/// </summary>
public static class IyzicoSignature
{
    /// <summary>
    ...
    public static (string Authorization, string RandomKey) Generate(
        string apiKey, string secretKey, string uriPath, string requestBody, string? randomKey = null)
    {
        ...
        var rnd = string.IsNullOrWhiteSpace(randomKey) ? RandomKey() : randomKey;
        var signature = Sign(secretKey, rnd + uriPath + requestBody);
        var raw = $"apiKey:{apiKey}&randomKey:{rnd}&signature:{signature}";
        return ("IYZWSv2 " + Convert.ToBase64String(Encoding.UTF8.GetBytes(raw)), rnd);
    }

    /// <summary>HMAC-SHA256 → KÜÇÜK harfli hex. (Base64 DEĞİL — iyzico hex bekler.)</summary>
    public static string Sign(string secretKey, string yuk)
    {
        using var hmac = new HMACSHA256(Encoding.UTF8.GetBytes(secretKey));
        var summary = hmac.ComputeHash(Encoding.UTF8.GetBytes(yuk));
        return Convert.ToHexStringLower(summary);
    }
