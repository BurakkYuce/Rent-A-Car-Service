...
export const QUOTATION_ID = '0b0e7c1a-8888-4aaa-8bbb-000000000008';
...
export function resRow(no: string, extra: Record<string, unknown> = {}) {
  return {
    id: RES_ID,
    no,
...
export function resDetail(
  ...
  return {
    ...
    musteriAd: 'Ayşe Yılmaz',
    plaka: '34 ABC 123',
    yetkiler: { duzenle: true, onayla: true, kirayaCevir: true, iptal: true, ...permission },
  };
}

export function quotationRow(no: string, extra: Record<string, unknown> = {}) {
  return {
    id: QUOTATION_ID,
    no,
    musteriId: CUSTOMER_ID,
...
export function quotationDetail(extra: Record<string, unknown> = {}) {
  const open =
    !('durum' in extra) || extra['durum'] === 'Taslak' || extra['durum'] === 'Gonderildi';
  return {
    teklif: {
      id: QUOTATION_ID,
      no: 'TK-000007',
    ...
    musteriAd: 'Ayşe Yılmaz',
    plaka: '34 ABC 123',
    yetkiler: { gonder: false, kabul: open, reddet: open },
  };
}

const sayfa = (records: unknown[]) => ({
  kayitlar: records,
...
export interface SahteSecenekler {
  ...
  readonly yazma?: (route: Route, request: Request) => Promise<void> | void;
...
export async function fakeReservationApi(page: Page, s: SahteSecenekler = {}): Promise<Sahte> {
  const record: Sahte = { listeIstekleri: [], detayIstekleri: [], teklifDetayIstekleri: [] };
  ...
  await page.route('**/api/ui/v1/secim/**', (route) => {
    ...
    if (path.endsWith('/musteri'))
      return route.fulfill({ json: [{ id: CUSTOMER_ID, etiket: 'Ayşe Yılmaz', tip: 'Bireysel' }] });
    if (path.endsWith('/arac'))
      return route.fulfill({
        json: [
          { id: VEHICLE_ID, etiket: '34 ABC 123', plaka: '34 ABC 123', grup: 'C', durum: 'Musait' },
        ],
      });
    if (path.endsWith('/lokasyon'))
      return route.fulfill({ json: [{ id: 'l-1', etiket: 'Merkez' }] });
    if (path.endsWith('/rezervasyon-kaynagi'))
  ...
  await page.route(
    ...
    async (route, request) => {
      if (request.method() === 'GET') {
        record.listeIstekleri.push(new URL(request.url()));
        return route.fulfill({ json: sayfa(s.rezSatirlari?.() ?? [resRow('RZ-000042')]) });
      }
      if (s.yazma) return s.yazma(route, request);
      return route.fulfill({ status: 201, json: { id: RES_ID, no: 'RZ-000042' } });
    },
  );
  await page.route(`**/api/ui/v1/rezervasyonlar/${RES_ID}`, async (route, request) => {
    if (request.method() === 'PUT') {
      if (s.yazma) return s.yazma(route, request);
      return route.fulfill({ json: resDetail({ surum: '813' }) });
    }
    record.detayIstekleri.push(Date.now());
    return route.fulfill({ json: s.detay?.() ?? resDetail() });
  });
  await page.route(`**/api/ui/v1/rezervasyonlar/${RES_ID}/onayla`, (route) =>
    route.fulfill({ json: resDetail({ durum: 'Onayli' }) }),
  );
  await page.route(`**/api/ui/v1/rezervasyonlar/${RES_ID}/kiraya-cevir`, (route) =>
    route.fulfill({ json: { kiraId: RENTAL_ID, sozlesmeNo: '2026230901001' } }),
  );
  await page.route(
    ...
    (route, request) =>
      request.method() === 'GET'
        ? route.fulfill({
            json: sayfa(s.teklifSatirlari?.() ?? [quotationRow('TK-000007')]),
          })
        : route.fulfill({ status: 201, json: { id: QUOTATION_ID, no: 'TK-000007' } }),
  );
  await page.route(`**/api/ui/v1/teklifler/${QUOTATION_ID}`, (route) => {
    record.teklifDetayIstekleri.push(Date.now());
    return route.fulfill({ json: s.teklifDetay?.() ?? quotationDetail() });
  });
  await page.route(`**/api/ui/v1/teklifler/${QUOTATION_ID}/kabul`, (route) =>
    s.teklifKabul
      ? s.teklifKabul(route)
      : route.fulfill({ json: { rezervasyonId: RES_ID, rezervasyonNo: 'RZ-000042' } }),
  );
  return record;
}
