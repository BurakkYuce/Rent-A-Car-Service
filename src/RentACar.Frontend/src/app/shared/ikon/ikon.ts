...
@Component({
  ...
  `,
  host: {
    '[style.width]': 'boyutCss()',
    '[style.height]': 'boyutCss()',
    '[class.don]': 'don()',
...
export class Ikon {
  ...
  readonly boyut = input<number | string>(IKON_BOYUTU.normal);
  readonly etiket = input<string>('');
  readonly don = input(false);

  protected readonly boyutCss = computed(() => {
    const deger = this.boyut();
