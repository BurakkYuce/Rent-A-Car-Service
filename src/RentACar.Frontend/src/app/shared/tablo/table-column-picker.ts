...
@Component({
  ...
  template: `
    <button
      ...
      (click)="acik() ? close(false) : openItem()"
    >
...
export class TableColumnPicker {
  ...
  protected openItem(): void {
    this.acik.set(true);
    afterNextRender(
      () => this.element.nativeElement.querySelector<HTMLInputElement>('.panel input')?.focus(),
  ...
  protected close(returnFocus: boolean): void {
    if (!this.acik()) return;
    this.acik.set(false);
    if (returnFocus) this.element.nativeElement.querySelector<HTMLButtonElement>('button')?.focus();
  }

  protected outsideClick(evt: MouseEvent): void {
    if (this.acik() && !this.element.nativeElement.contains(evt.target as Node)) {
      this.close(false);
    }
  }
}
