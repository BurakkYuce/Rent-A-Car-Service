...
import { type FiloListeSatiri, sayi } from './filo-modeli';
 ...
 */
export function filoSutunlari(t: Ceviri): readonly TabloSutunu<FiloListeSatiri>[] {
  const s = (kod: SutunKodu) => t(`filoKiralama.sutun.${kod}`);
  const doviz = (r: FiloListeSatiri) => r.doviz || 'TRY';
  return [
