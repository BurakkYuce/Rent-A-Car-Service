...
@Component({
  ...
  template: `
    <section class="rc-bolum" aria-labelledby="yol-plaka">
      ...
      <div class="rc-arac-cubugu">
        <rc-plaka-arama (ara)="searched.set($event)" />
        <span role="status">
          @if (searched(); as a) {
    ...
    <section class="rc-bolum" aria-labelledby="yol-liste">
      ...
      <rc-gorunum-cipleri [gorunumler]="views()" (secildi)="selectView($event)" />
      <rc-filtre-paneli
        depoAnahtari="rc.vitrin.filtre"
        (temizle)="evt.set('Temizlendi')"
        (filtrele)="evt.set('Filtrelendi')"
      >
        <label class="rc-alan">
      ...
      <p class="rc-hucre-alt" role="status">{{ evt() }}</p>
...
export class PathComponents {
  protected readonly searched = signal<NormalizedPlate | null>(null);
  protected readonly evt = signal('');
  protected readonly views = signal<readonly SavedView[]>(VIEWS);

  protected selectView(selected: SavedView): void {
    this.views.update((list) => list.map((g) => ({ ...g, aktif: g.id === selected.id })));
  }
}
