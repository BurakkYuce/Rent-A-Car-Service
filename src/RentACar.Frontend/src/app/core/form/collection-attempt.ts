...
import type { ApiHatasi, CurrentOperation } from '../api/api-hatasi';
import { formatMoney } from '../bicim/bicim';
import type { ToastService } from '../geri-bildirim/toast-service';
 ...
 *   tutar temizlenir.
 */
export type CollectionDuplicateType =
  | 'zatenKaydedildi'
 ...
 */
export function unknownOutcomeError(error: ApiHatasi): boolean {
  return error.kod === 'ag' || error.kod === 'sunucu' || error.kod === 'bilinmeyen';
}

function toNumber(d: number | string | null | undefined): number | null {
  if (d === null || d === undefined || d === '') return null;
  const n = typeof d === 'number' ? d : Number(d);
  return Number.isFinite(n) ? n : null;
}

const normalizedCurrency = (d: string) => d.trim().toLocaleUpperCase('tr-TR');

/** `mevcut` bu denemenin kaydı olabilir mi: tutar + döviz (sunucu `mevcut`'ta hesap türü vermez). */
export function matchesAttempt(existing: CurrentOperation, d: TahsilatIcerigi): boolean {
  const a = toNumber(existing.tutar);
  return (
    a !== null &&
    a === toNumber(d.tutar) &&
    normalizedCurrency(existing.doviz) === normalizedCurrency(d.doviz)
  );
}
 ...
 */
export const COLLECTION_ATTEMPT_CHANNEL = new InjectionToken<string | null>(
...
function extractContent(x: unknown): TahsilatIcerigi | null {
  if (typeof x !== 'object' || x === null) return null;
  const { tutar: amount, doviz: currency, hesap: account } = x as Record<string, unknown>;
  const isAmountValid = amount === null || typeof amount === 'string' || typeof amount === 'number';
  if (
    !isAmountValid ||
    ...
    return null;
  return { tutar: amount, doviz: currency, hesap: account };
}

function openChannel(name: string | null): BroadcastChannel | null {
  return name === null || typeof BroadcastChannel === 'undefined'
    ? null
    : new BroadcastChannel(name);
}

const sameContent = (a: TahsilatIcerigi, b: TahsilatIcerigi) =>
...
export class CollectionAttemptRecord {
  private readonly unknowns = new Map<string, TahsilatIcerigi[]>();
  private readonly kanal: BroadcastChannel | null = openChannel(inject(COLLECTION_ATTEMPT_CHANNEL));

  constructor() {
    inject(SessionService, { optional: true })?.registerCleanup(() => {
      this.unknowns.clear();
      this.yay({ tur: 'temizle' });
    });
    inject(DestroyRef, { optional: true })?.onDestroy(() => this.closeChannel());
    if (this.kanal) {
      this.kanal.onmessage = (e: MessageEvent<unknown>) => this.get(e.data);
      this.yay({ tur: 'senkronIste' });
    }
  }

  /** Bu anahtarla sonucu bilinmeyen denemeler (anlık kopya; öğeler aynı nesneler). */
  bilinmeyen(key: string): TahsilatIcerigi[] {
    return [...(this.unknowns.get(key) ?? [])];
  }

  ekle(key: string, content: TahsilatIcerigi): void {
    this.addLocal(key, content);
    this.yay({
      tur: 'ekle',
      anahtar: key,
      icerik: { tutar: content.tutar, doviz: content.doviz, hesap: content.hesap },
    });
  }
  ...
  close(key: string): void {
    ...
    this.yay({ tur: 'kapat', anahtar: key });
  }
  ...
  private addLocal(key: string, content: TahsilatIcerigi): void {
    const list = this.unknowns.get(key) ?? [];
    list.push(content);
    this.unknowns.set(key, list);
  }

  private yay(m: ChannelMessage): void {
    try {
  ...
  private get(data: unknown): void {
    if (typeof data !== 'object' || data === null) return;
    const m = data as Record<string, unknown>;
    switch (m['tur']) {
      case 'ekle': {
        const content = extractContent(m['icerik']);
        if (typeof m['anahtar'] === 'string' && content) this.addLocal(m['anahtar'], content);
        return;
      }
      case 'kapat':
        if (typeof m['anahtar'] === 'string') this.unknowns.delete(m['anahtar']);
        return;
      case 'temizle':
        this.unknowns.clear();
        return;
      case 'senkronIste':
        if (this.unknowns.size > 0)
          this.yay({
            tur: 'durum',
            kayitlar: [...this.unknowns].map(([a, l]) => [a, [...l]] as const),
          });
        return;
      case 'durum':
        if (Array.isArray(m['kayitlar'])) this.mergeState(m['kayitlar']);
        return;
    }
  }

  /** Birden çok sekme aynı durumu yanıtlayabilir: aynı içerik ikinci kez eklenmez. */
  private mergeState(records: unknown[]): void {
    for (const k of records) {
      if (!Array.isArray(k) || typeof k[0] !== 'string' || !Array.isArray(k[1])) continue;
      const key = k[0];
      for (const raw of k[1] as unknown[]) {
        const content = extractContent(raw);
        if (content && !this.bilinmeyen(key).some((d) => sameContent(d, content)))
          this.addLocal(key, content);
      }
    }
  }
 ...
 * kaydı paylaşımlı {@link CollectionAttemptRecord}'dadır; bu sınıf yalnız formun L-2 bayrağını taşır.
 *
...
export class CollectionAttempt {
  private amountToRefresh: string | null = null;

  constructor(private readonly record: CollectionAttemptRecord) {}

  start(key: string, content: TahsilatIcerigi): TahsilatGonderimi {
    return { anahtar: key, icerik: content, onceki: this.record.bilinmeyen(key) };
  }

  /** Bu anahtarla sonucu bilinmeyen bir deneme var mı (herhangi bir formdan). */
  isRepeat(key: string | null | undefined): boolean {
    return key != null && this.record.bilinmeyen(key).length > 0;
  }

  successful(g: TahsilatGonderimi): void {
    this.record.close(g.anahtar);
    this.amountToRefresh = null;
  }
   ...
   */
  errorReceived(g: TahsilatGonderimi, error: ApiHatasi): CollectionDuplicateType | null {
    if (unknownOutcomeError(error)) {
      this.record.ekle(g.anahtar, g.icerik);
      return null;
    }
    if (error.kod !== 'mukerrer') return null;
    for (const d of this.record.bilinmeyen(g.anahtar)) if (!g.onceki.includes(d)) g.onceki.push(d);
    const m = error.mevcut;
    if (!m) return 'bayatAnahtar';
    if (m.ayniIcerik) return 'zatenKaydedildi';
    if (g.onceki.some((d) => matchesAttempt(m, d))) return 'oncekiDenemeKaydedilmis';
  ...
  requestAmountRefresh(key: string): void {
    this.amountToRefresh = key;
  }

  /** Tazelenen kaydın anahtarı geldi: tutar yeni öneriyle yenilenmeli mi (bir kez `true`). */
  shouldRefreshAmount(newKey: string | null | undefined): boolean {
    if (this.amountToRefresh === null || newKey == null) return false;
    if (newKey === this.amountToRefresh) return false;
    this.amountToRefresh = null;
    return true;
  }
}
...
export function amountCleared(type: CollectionDuplicateType): boolean {
  return (
    type === 'oncekiDenemeKaydedilmis' ||
 ...
 */
export function reportCollectionDuplicate(
  toast: ToastService,
  t: Translation,
  type: CollectionDuplicateType,
  error: ApiHatasi,
  b: MukerrerBildirimi,
): void {
  const attached = (text: string) => (b.ek ? `${text} ${b.ek}` : text);
  const g = b.gonderim;
  switch (type) {
    case 'zatenKaydedildi':
      toast.bilgi(attached(error.detay), { baslik: t('geriBildirim.zatenKaydedildi') });
      return;
    case 'oncekiDenemeKaydedilmis': {
      const m = error.mevcut;
      const text = t('geriBildirim.oncekiDenemeKaydedildi', {
        no: m?.belgeNo ?? '',
        kayitli: formatMoney(toNumber(m?.tutar), m?.doviz),
        girilen: formatMoney(toNumber(g.icerik.tutar), g.icerik.doviz),
      });
      toast.uyari(attached(text), { baslik: t('geriBildirim.oncekiDenemeBaslik') });
      return;
    }
    case 'baskaIslemDenemeYazilmadi': {
      const previous = [...new Set(g.onceki.map((d) => formatMoney(toNumber(d.tutar), d.doviz)))];
      const text = `${error.detay} ${t('geriBildirim.denemeKaydedilmedi', { onceki: previous.join(', ') })}`;
      toast.uyari(attached(text), { baslik: t('geriBildirim.baskaIslemYazildi') });
      return;
    }
    case 'baskaIslemYazildi':
      toast.uyari(attached(error.detay), { baslik: t('geriBildirim.baskaIslemYazildi') });
      return;
    case 'bayatAnahtar':
      toast.uyari(attached(error.detay), { baslik: b.bayatBaslik });
      return;
  }
}
