...
|---|---|
| Defter kümesi (`Infrastructure/Persistence/LedgerSet.cs`: `LedgerPoster` ve depozito) | hesap türü, hesap referansı (cari/kasa/banka), yön, tutar, döviz ve kur, DB hassasiyetine yuvarlanmış halde. Depozito iratında ayrıca kira atfı. |
...
| E32 | `VehicleLoanService.TaksitOdeAsync` `AracKredileri/VehicleLoanService.cs:74` | IslemAnahtari → `Expense.IslemAnahtari` (E21 index) + `FOR UPDATE` | Anahtarlı: **409** "Bu taksit ödemesi zaten kaydedilmiş (çift gönderim)." — son taksitte de. **F1.4 değişikliği** (önce: ara taksitte 400 [catch `IdempotencyConstraint`'ye bağlı değildi], son taksitte sessiz `false`). Anahtarsız: sonraki taksidi öder; hepsi ödendiyse `false` | anahtar önce (`VehicleLoanRepository.cs:87`) | doğrulandı (test: ara + son taksit) |
...
| E36 | `PeriodClosingVoucherService.KapatAsync` `Periods/PeriodClosingVoucherService.cs:42` | Yapısal: kapanış tarihi + kiracı danışma kilidi (`PeriodClosingRepository.cs:27`) | **400** "Dönem zaten … tarihine kapalı. …" (yarışta aynı metin) | ön-kontrol + kilit | doğrulandı (test) |
...
| durum geçişleri (sipariş, BAF, hasar, filo delta) | yapısal | yok | satır kilidi altında durum çiti (`RowVersionSql`); PUT'lar zorunlu `surum` → 409 `cakisma` |
